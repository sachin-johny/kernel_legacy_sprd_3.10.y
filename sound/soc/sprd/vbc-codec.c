...
static void vbc_amplifier_enable(int enable, const char *prename);
static int vbc_amplifier_enabled(void);
static struct pdata_notifier *pdata_notifier_amplifier;
...
#elif !VBC_EQ_MODULE_SUPPORT
            vbc_reg_VBCR1_set(BTL_MUTE, 0); // unMute earpiece
            vbc_reg_VBCR1_set(HP_DIS, 0); // unMute headphone
            vbc_amplifier_enable(true, "force"); // unMute speaker
            pr_info("....................... turn on all PA .......................");
#endif
            // vbc_reg_write(VBCGR1, 0, VBCGR1_value, 0xff); // DAC Gain
        }
#if !VBC_DYNAMIC_POWER_MANAGEMENT
...
static void vbc_amplifier_enable(int enable, const char *prename)
{
    ...
    pr_debug("[headset_muted =%d]\n"
           ...
           "[speaker_muted =%d]\n", headset_muted, earpiece_muted, speaker_muted);
#endif
    if (pdata_notifier_amplifier)
        pdata_notifier_amplifier->notifier_call(pdata_notifier_amplifier, enable, NULL);
...
{
    if (pdata_notifier_amplifier) {
        return pdata_notifier_amplifier->notifier_call(pdata_notifier_amplifier, -1, NULL);
    } else {
#if defined(CONFIG_ARCH_SC8800G)
...
{
    pdata_notifier_amplifier = platform_get_drvdata(pdev);
    return 0;
}

static int __devexit speaker_pa_remove(struct platform_device *pdev)
{
    pdata_notifier_amplifier = NULL;
    return 0;
}
...
{
    if (platform_driver_register(&speaker_pa_driver))
        pr_err("register platform speaker-pa driver failed\n");
