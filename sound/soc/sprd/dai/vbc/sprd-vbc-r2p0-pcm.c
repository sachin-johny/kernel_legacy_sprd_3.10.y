...
static int sprd_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if (!((substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		...
		memset(rtd->dma_desc_array, 0x0, 4 * PAGE_SIZE);
#ifdef CONFIG_SPRD_AUDIO_BUFFER_USE_IRAM
	} else {
...
#endif
		dma_free_coherent(substream->pcm->card->dev, 4 * PAGE_SIZE,
				  rtd->dma_desc_array,
				  rtd->dma_desc_array_phys);
	kfree(rtd);
			      ...
			      struct snd_pcm_hw_params *params)
{
	...
	do {
		for (i = 0; i < used_chan_count; i++) {
			...
			dma_desc[i]->fragmens_len = dma->desc.fragmens_len;
	...
	dma_reg_addr[0].phys_addr =
	    (u32) (rtd->dma_desc_array_phys) + 2 * PAGE_SIZE;
	dma_reg_addr[0].virt_addr = (u32) (rtd->dma_desc_array) + 2 * PAGE_SIZE;
	...
	if (used_chan_count > 1) {
		dma_reg_addr[1].phys_addr =
		    (u32) (rtd->dma_desc_array_phys) + 3 * PAGE_SIZE;
		dma_reg_addr[1].virt_addr =
		    (u32) (rtd->dma_desc_array) + 3 * PAGE_SIZE;
