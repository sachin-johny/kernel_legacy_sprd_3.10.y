...
static int dcdc_get_voltage(struct regulator_dev *rdev)
{
	...
	if (regs->vol_ctl) {
		...
		i = (ANA_REG_GET(regs->vol_ctl) & regs->
		     vol_ctl_bits) >> shft_ctl;
		uV = regs->vol_sel[i];

		if (regs->vol_trm) {
			cal =
			    (ANA_REG_GET(regs->vol_trm) & regs->
			     vol_trm_bits) >> shft_trm;
				  ...
				  int sz)
{
	...
	debug
	    ("[%d] %s type %d, range %d(uV) - %d(uV), step %d(uV), default %d(uV) - (%s)\n",
	     (idx.counter - 1), np->name, type,
	     desc->init_data->constraints.min_uV,
	     desc->init_data->constraints.max_uV, regs->step_uV, regs->vol_def,
	     (regs->typ & BIT(4)) ? "on" : "off");
...
static int sci_regulator_register_dt(struct platform_device *pdev)
{
	...
	for_each_child_of_node(dev_np, child_np) {
		...
		ret =
		    sci_regulator_parse_dt(pdev, child_np, sci_desc,
