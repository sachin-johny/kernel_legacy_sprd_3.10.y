	...
	if(auth_alg==2)
	{
		...
		else
		{
			...
			if(pattrib->bdecrypted==0)
				RT_TRACE(_module_rtl871x_recv_c_,_drv_err_,("portctrl:prxstat->decrypted=%x\n", pattrib->bdecrypted));

			prtnframe=precv_frame;
			//check is the EAPOL frame or not (Rekey)
			if(ether_type == eapol_type){

				RT_TRACE(_module_rtl871x_recv_c_,_drv_info_,("########portctrl:ether_type == 0x888e\n"));
				//check Rekey

				prtnframe=precv_frame;
			}
			else{
				RT_TRACE(_module_rtl871x_recv_c_,_drv_info_,("########portctrl:ether_type = 0x%.4x\n",ether_type));
			}
		}
	}
	else
...
_func_enter_;
	...
	if(pattrib->order)//HT-CTRL 11n
	{
		...
		{
			RT_TRACE(_module_rtl871x_recv_c_,_drv_notice_,("decache : drop pkt\n"));
			ret= _FAIL;
			goto exit;
		}
#ifdef CONFIG_TDLS
	...
	_rtw_memcpy(&ether_type, ptr, 2);
	ether_type= ntohs((unsigned short )ether_type);
	if (ether_type == ETH_P_IP && pfhdr->len > 282) {
		u8 *tmp;
	...
	if(!pattrib->amsdu)
	{
			...
			|| (pattrib->eth_type==0x0806) || (pattrib->ack_policy!=0))
		{
			if ((padapter->bDriverStopped == _FALSE) &&
			    (padapter->bSurpriseRemoved == _FALSE))
			{
				RT_TRACE(_module_rtl871x_recv_c_, _drv_info_, ("@@@@  recv_indicatepkt_reorder -recv_func recv_indicatepkt\n" ));
...
int recv_func(_adapter *padapter, void *pcontext)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_MP_STATE) == _TRUE))//&&(padapter->mppriv.check_mp_pkt == 0))
	{
		...
		if (check_fwstate(pmlmepriv, WIFI_MP_LPBK_STATE) == _FALSE) {
			RT_TRACE(_module_rtl871x_recv_c_, _drv_info_, ("MP - Not in loopback mode , drop pkt \n"));
			retval = _FAIL;
