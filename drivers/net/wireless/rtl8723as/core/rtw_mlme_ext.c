		...
		if (pmlmeinfo->state & WIFI_FW_AUTH_NULL)
		{
			...
			if (pbss) {
				if (collect_bss_info(padapter, precv_frame, pbss) == _SUCCESS) {
					update_network(&(pmlmepriv->cur_network.network), pbss, padapter);
...
void _linked_rx_signal_strehgth_display(_adapter *padapter)
{
	...
	if((pmlmeinfo->state&0x03) == WIFI_FW_STATION_STATE)
	{
		...
		rtw_hal_get_def_var(padapter, HW_DEF_RA_INFO_DUMP,&UndecoratedSmoothedPWDB);
	}
	...
	if (is_client_associated_to_ap(padapter))
	{
		...
		if ((psta = rtw_get_stainfo(pstapriv, pmlmeinfo->network.MacAddress)) != NULL)
		{
			...
			if (sta_last_rx_pkts(psta) == sta_rx_pkts(psta))
			{
				...
				if(pmlmeext->retry<1)
				#else
				if(pmlmeext->retry<ROAMING_LIMIT)// Alter the retry limit to 8
				#endif
				{
					if(pmlmeext->retry==0)
					{
