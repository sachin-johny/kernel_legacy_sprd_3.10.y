...
u32 sdio_init_irq(PADAPTER padapter)
{
	struct dvobj_priv *psddev;
	PSDIO_DATA psdio_data;
	struct sdio_func *func;
...
_func_enter_;
	...
	if (err) {
		DBG_8192C(KERN_CRIT "%s: sdio_claim_irq FAIL(%d)!\n", __func__, err);
	}
	sdio_release_host(func);
_func_exit_;
...
void sdio_deinit_irq(PADAPTER padapter)
{
	struct dvobj_priv *psddev;
	struct sdio_func *func;
	int err;
	...
	const struct sdio_device_id *id)
{
	...
	padapter->intf_start_irq = &sdio_init_irq;
	padapter->intf_stop_irq = &sdio_deinit_irq;
...
/* this function is used for open/close sdio clock & power */
...
exit:
	DBG_871X_LEVEL(_drv_always_, "module init success\n");
