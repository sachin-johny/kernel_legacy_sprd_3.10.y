...
#if defined (CONFIG_ARCH_SC) || (LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 39))
	switch (action)	{
...
#ifdef DHDTHREAD
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
    if (dhd->thr_wdt_ctl.p_task->pid >= 0) {
...
#ifdef DHDTHREAD
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
        if (dhd->thr_dpc_ctl.p_task->pid >= 0) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
    PROC_START(dhd_watchdog_thread, dhd, &dhd->thr_wdt_ctl, "dhd_watchdog");
    PROC_START(dhd_dpc_thread, dhd, &dhd->thr_dpc_ctl, "dhd_dpc");
#else
    if (dhd_dpc_prio >= 0) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
    PROC_START(_dhd_sysioc_thread, dhd, &dhd->thr_sysioc_ctl, "dhd_sysioc");
#else
    if (dhd_sysioc) {
...
int dhd_spi_reset(struct net_device *dev)
{
	dhd_info_t *dhd = *(dhd_info_t **)netdev_priv(dev);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
#ifdef DHD_REOPEN
		PROC_START(dhd_reopen_thread, dev, &dhd->thr_reopen_ctl, "dhd_reopen");
#else
