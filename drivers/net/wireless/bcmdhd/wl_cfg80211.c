	...
	struct vif_params *params)
{
	...
	case NL80211_IFTYPE_MONITOR:
         wl->wdev->netdev = wl_cfg80211_add_monitor_if(name);
	...
	unsigned int duration, u64 *cookie)
{
	...
	if (id == 0)
		id = ++wl->last_roc_id;
	*cookie = id;
	cfg80211_ready_on_channel(wl->wdev, *cookie, channel, duration, GFP_KERNEL);
	if (wl->p2p && !wl->p2p->on) {
		get_primary_mac(wl, &primary_mac);
...
#endif
#if defined(WL_SUPPORT_BACKPORTED_KPATCHES) || (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 8))
...
static s32 wl_inform_single_bss(struct wl_priv *wl, struct wl_bss_info *bi)
{
	...
	if (wl->p2p_net && wl->scan_request &&
		...
		(wl->scan_request->wdev->netdev == wl_to_p2p_bss_ndev(wl, P2PAPI_BSSCFG_CONNECTION)))) {
#else
...
#else
		cfg80211_rx_mgmt(wl->wdev, freq, mgmt_frame, len, GFP_ATOMIC);
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0) */
...
#else
	cfg80211_rx_mgmt(wl->wdev, freq, mgmt_frame, mgmt_frame_len, GFP_ATOMIC);
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0) */
...
static s32 wl_create_event_handler(struct wl_priv *wl)
{
    ...
    PROC_START(wl_event_handler, wl, &wl->event_tsk, "wl_event_handler");
#else
