...
thr_wait_for_2nd_eth_dev(void *data)
{
	...
	iw = *(wl_iw_t **)netdev_priv(dev);
        ...
        DAEMONIZE("wl0_eth_wthread");
            ...
            complete(&tsk_ctl->completed);
#endif
printk("thr_wait_for_2nd_eth_dev\n");
...
set_ap_cfg(struct net_device *dev, struct ap_profile *ap)
{
	...
	if (ap_cfg_running == FALSE) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
    PROC_START(thr_wait_for_2nd_eth_dev, dev, &ap_eth_ctl, "thr_wait_for_2nd_eth_dev");
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
    PROC_START(_bt_dhcp_sysioc_thread, bt_dhcp, &bt_dhcp->tsk_ctl, "bt_dhcp_sysioc");
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
    PROC_START(_iscan_sysioc_thread, iscan, &iscan->tsk_ctl, "iscan_sysioc");
#else
