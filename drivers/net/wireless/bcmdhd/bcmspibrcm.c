...
static int bcmspi_client_init(sdioh_info_t *sd);
static sdioh_info_t *reset_sd = NULL;
int
...
#define CTRL_REG_DEFAULT	0x00010430 /* according to the host m/c */
	...
	if ((status = bcmspi_card_regread(sd, 0, SPID_CONFIG, 4, &regdata)) != SUCCESS) {
			printk("status = %8x, regdata=%08x \n", status, regdata);
			return FALSE;
	}
	else
			printf("check regdata = %x\n",regdata);  
