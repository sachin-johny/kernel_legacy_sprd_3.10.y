...
extern struct spi_device *get_wifi_device(int master_bus_num);
extern int spi_write_wifi(struct spi_device *spi, const u8 *buf, size_t len);
extern int spi_read_wifi(struct spi_device *spi, const u8 *buf, size_t len);
extern  int wifi_spi_sync(unsigned char *data1, int len1, int dir1,
...
//Start to add by Gat ------------------------2011/03/22
#if 0
...
{
#if 0
	spih_info_t *si = (spih_info_t *if (bcmpcispi_dump) {
...
#ifdef BCMSPI
#define GSPI_MAX_CLK	48000000
		if ((div == 1) && (xtal_freq > GSPI_MAX_CLK)) {
			div = 2;
			sd_err(("%s: Dividing xtal freq by 2 as gSPI bus can work"
			        " at <= 48MHz. \n", __FUNCTION__));
			sd_err(("%s: Clock is %dHz\n", __FUNCTION__, xtal_freq/div));
			ASSERT(xtal_freq/div < GSPI_MAX_CLK);
		}
#endif /* BCMSPI */
...
/* Add platform disable intr here*/
...
extern int gg_wordlen;
extern void setgwordlen(int value,int t);
extern int getfun(int t);
extern int setfunction(int t);
void
...
{	
	...
	if (1){	
		...
		if (Write){
			...
			t[1].rx_buf = (char *)&msg_in[msglen-4];
			t[1].tx_buf = 0;
			t[1].len = 4;
