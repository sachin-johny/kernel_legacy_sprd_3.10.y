...
static int rt8973_get_cable_type_by_id(rt8973_chip_t *chip)
{
	...
	if ((chip->prev_status.cable_type ==
		...
		chip->curr_status.dcdt_status = 0;
	}

	return cable_type;
}

#define USB_RETRY_CNT 10
#define USB_RETRY_DELAY 50

static int rt8973_get_cable_type(rt8973_chip_t *chip)
{
	int i, ret = -1;
	int dev_type_retry = 1;
	/* Check dangerous case first and
	 * * it can make system stop charging */
	if (chip->curr_status.ovp_status |
			chip->curr_status.ocp_status)
		return MUIC_RT8973_CABLE_TYPE_UNKNOWN;
	/* Only for RT8973A */
	if (chip->adc_reg_addr == 0x23) {
		for (i = 0; i < USB_RETRY_CNT && dev_type_retry == 1;
				i++) {
			ret = rt8973_get_cable_type_by_device_reg(chip);
			/* VBUS = 1 but no DCD_T,
			 * * and not a SDP/CDP/DCP type cable
			 * * and not a L-type USB cable (200k-Ohm) */
			if (ret < 0 && chip->curr_status.vbus_status == 1 &&
					chip->curr_status.dcdt_status == 0 &&
					chip->curr_status.id_adc != 0x17) {
				msleep(USB_RETRY_DELAY);
				rt8973_read(chip, RT8973_REG_DEVICE1,
						2, chip->curr_status.device_reg);
				pr_info("%s : retry to read DEV1 = 0x%x, DEV2 = 0x%x\n",
						__func__, (int)chip->curr_status.device_reg[0],
						(int)chip->curr_status.device_reg[1]);
			} else {
				dev_type_retry = 0;
			}
		...
		const struct i2c_device_id *id)
{
	...
	ret = request_irq(chip->irq, rt8973_irq_handler,
			RT8973_IRQF_MODE | IRQF_NO_SUSPEND,
