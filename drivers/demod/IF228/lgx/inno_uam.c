...
static int uam_send_cmd_handler(void *data)
{
        ...
        if (inno_comm_cmd_idle()) {
                if (len > 1)
                        inno_comm_send_unit(UAM_BASE_ADDR, (cmd+1), (len-1));
...
static int uam_get_rsp(unsigned char *rsp, int *len)
{
        ...
        if (uam_dev->valid == 0) {
                ...
                pr_debug("%s:wait for rsp irq\n", __func__); 
                if (wait_for_completion_interruptible(&done)) {
                        pr_debug("%s:wait failed\n",__func__);
                        return -ERESTARTSYS;
                }
...
static int uam_set_mbbms_isma(unsigned char isBase64, struct cmbbms_isma *isma)
{
        ...
        cmd[9] = (unsigned char)isma->MBBMS_ECMDataType;
        memcpy(cmd+10, isma->ISMACrypAVSK[0].ISMACrypSalt,18);
        cmd[28] = isma->ISMACrypAVSK[0].SKLength;   
        memcpy(cmd+29, isma->ISMACrypAVSK[1].ISMACrypSalt,18);
        cmd[47] = isma->ISMACrypAVSK[1].SKLength;

        //OutputStr(cmd+9, 20);
        //OutputStr(cmd+29, 19);
        
        for (retry = 0; retry < 20; retry ++) {
                ret = uam_send_cmd(cmd, 48);
                if (ret < 0)
                        continue;
                else 
                        break;
...
//add by mahanghong 20110210
static int uam_set_cw(struct cmbbms_cw *cw)
{
        ...
        memcpy(cmd+10, cw->CW_DATA,16);

        //OutputStr(cmd+9, 20);
        //OutputStr(cmd+29, 19);
        
        for (retry = 0; retry < 20; retry ++) {
                ret = uam_send_cmd(cmd, 26);
                if (ret < 0)
                        continue;
                else 
                        break;
...
static int uam_set_aid_3g(struct aid_3g *aid)
{
		...
		memcpy(cmd+9, aid->AID_DATA, aid->AIDLength);
		
        for (retry = 0; retry < 20; retry ++) {
       			ret = uam_send_cmd(cmd, aid->AIDLength+9);
        		if (ret < 0)
                		continue;
       			else 
               			break;
        ...
        for(retry = 0; retry < 3; retry++) {
                ...
                for(i=0;i<1000;i++) {
  				     ...
  				     //modified by mahanghong 20110118
                                     //inno_comm_get_unit(UAM_BASE_ADDR,info->output,64); 
			...
			pr_debug("%s:param.sw = %x\n",__func__, param.sw); 
                ...
                case INNO_IO_UAM_MBBMS_ISMA:
                {
                        ...
                        if (copy_from_user(&isma_param, argp, sizeof(isma_param)))
                                return -EFAULT;
                        if (uam_set_mbbms_isma(isma_param.isbase64, &isma_param.mbbms_isma))
                                return -EIO;        
                        break;
                }
				//add by mahanghong 20110210
                ...
                {
                        struct cmbbms_cw cw_param;
                        if (copy_from_user(&cw_param, argp, sizeof(cw_param)))
                                return -EFAULT;
                        if (uam_set_cw(&cw_param))
                                return -EIO;        
                        break;
                }
				//add by mahanghong 20110218
