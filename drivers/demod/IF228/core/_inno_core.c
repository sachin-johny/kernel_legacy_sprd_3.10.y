...
{
        lgxinfo.lgx_info = (struct inno_lgx_info *)kmalloc(n * sizeof *info, GFP_KERNEL);
        if (!lgxinfo.lgx_info)
                return;
...
struct lgx_device *inno_lgx_alloc_device(char *name, int major, int id, void *data)
{
        ...
        lgx = kzalloc(sizeof(*lgx) + strlen(name), GFP_KERNEL);
        if (!lgx) {
...
static int inno_demod_probe(struct platform_device *pdev)
{
        ...
        inno_demod = kzalloc(sizeof(*inno_demod), GFP_KERNEL); 
        if (inno_demod == NULL) {
                ret = -ENOMEM;
                goto err1;
        }
