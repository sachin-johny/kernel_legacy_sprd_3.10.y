...
static int
vshm_ioctl (struct inode* inode, struct file* file, unsigned int cmd, unsigned long arg)
{
    int err = 0;
...
vshm_module_init (void)
{
    ...
    cls_dev = device_create(vshm_class, NULL, MKDEV(VSHM_MAJOR, VSHM_MINOR), NULL, VSHM_NAME);
    if (IS_ERR(cls_dev)) {
    ...
    if (err != 0) {
	device_destroy(vshm_class, MKDEV(VSHM_MAJOR, VSHM_MINOR));
	class_destroy(vshm_class);
	return -EIO;
    }
