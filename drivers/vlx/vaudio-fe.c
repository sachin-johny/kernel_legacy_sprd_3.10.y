 ...
 *	       communication layer
 *
 * Copyright (C) 2005-2010, VirtualLogix. All Rights Reserved.
 *
 ...
 *
 ****************************************************************
 * #ident  "@(#)vaudio-fe.c 1.12     08/03/28 VirtualLogix"
 ****************************************************************
 */
...
vaudio_snd_init_card (NkVaudio dev)
{
	...
	for (j = 0; j < NK_VAUDIO_STREAM_MAX; j++) {
	    ...
	    if (config->pcm.buffer_bytes_max > NK_VAUDIO_MAX_RING_SIZE) {
		config->pcm.buffer_bytes_max = NK_VAUDIO_MAX_RING_SIZE;
	    }
	    ...
	    if (config->pcm.periods_max > NK_VAUDIO_RING_DESC_NB) {
		config->pcm.periods_max = NK_VAUDIO_RING_DESC_NB;
	    }
