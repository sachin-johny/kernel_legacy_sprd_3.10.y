 ...
 ****************************************************************
 *
 *  Component:	VirtualLogix virtual video BE
 *
...
static struct device* vvideo_hw_device[ VVIDEO_HW_DEV_MAX + 1 ] = { NULL };
...
vvideo_hw_init (void)
{
	...
	dev = device_create(vvideo_hw_class, NULL, MKDEV(VVIDEO_HW_MAJOR, minor),
...
vvideo_hw_deinit (void)
{
	...
	if (vvideo_hw_device[ minor ]) {
	    device_destroy(vvideo_hw_class, MKDEV(VVIDEO_HW_MAJOR, minor));
	}
    }
