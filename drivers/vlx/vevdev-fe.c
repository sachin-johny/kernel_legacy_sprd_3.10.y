 ...
 *
 ****************************************************************
 * #ident  "@(#)vevdev-fe.c 1.5     08/06/18 VirtualLogix"
 ****************************************************************
 */
...
static void vdev_get_config(VIdev* videv, const char *name, struct input_dev *dev)
{
	...
	if (strcmp(name, VTS_NAME) == 0) {
		...
		size = (ABS_CNT > VEVENT_ABS_CNT) ?
				...
				(sizeof(int) * (ABS_CNT));
		memcpy(dev->absmax, videv->absmax, size);
		memcpy(dev->absmin, videv->absmin, size);
		memcpy(dev->absfuzz, videv->absfuzz, size);
		memcpy(dev->absflat, videv->absflat, size);
	}
