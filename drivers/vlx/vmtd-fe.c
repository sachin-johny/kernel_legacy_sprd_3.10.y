 ...
 ****************************************************************
 *
 *  Component:	VirtualLogix VMTD-FE
 *
...
static spinlock_t    vmtd_spinlock = SPIN_LOCK_UNLOCKED;
...
vmtd_put_device (struct mtd_info* mtd)
{
    ...
    if (vmtd->deleted) {
	     ...
	     *  calling del_mtd_device() would deadlock.
	     */
...
#define VMTD_FIELD(x)	mtd->x = remote->x
	...
	/* add_mtd_device() will set mtd->index (local value) */
...
restart:
	...
	if (del_mtd_device (&vmtd->local_mtd)) {
...
restart:
	...
	if (link && (vmtd->link != link ||
	    ...
	    continue;
	DTRACE ("deleting remote index %d\n", vmtd->remote_mtd.index);
	if (vmtd->added && del_mtd_device (&vmtd->local_mtd)) {
	    vmtd->deleted = true;
...
error:
     ...
     *  during add_mtd_device(), hence crash.
     */
