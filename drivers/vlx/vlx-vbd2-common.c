...
vbd_cb_get_xx_config (vmq_link_t* link, const char* start)
{
	 ...
	 * string to find it.
	 */
    if (!start || start == vmq_link_tx_s_info (link)) {
	const char* rx_s_info = vmq_link_rx_s_info (link);
