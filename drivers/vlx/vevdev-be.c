 ...
 ****************************************************************
 *
 * Copyright (C) 2008 VirtualLogix inc. All Rights Reserved.
 *
 * Replace evdev handler on backend side
 *
 ****************************************************************
 * #ident  "@(#)vevdev-be.c 1.10     08/06/18 VirtualLogix"
 ****************************************************************
 */
...
MODULE_AUTHOR("guennadi.maslov@virtuallogix.com");
...
init_focus_task (void)
{
	...
	/* init semaphore */
    init_MUTEX_LOCKED(&wait_focus_task);
...
evdev_get_config (VIdev* videv, char *name, struct input_dev *dev)
{
	...
	if (strcmp(name, VTS_NAME) == 0) {
		...
		size = (ABS_CNT > VEVENT_ABS_CNT) ?
				(sizeof(int) * (VEVENT_ABS_CNT)) :
				(sizeof(int) * (ABS_CNT));
		memcpy(videv->absmax, dev->absmax, size);
		memcpy(videv->absmin, dev->absmin, size);
		memcpy(videv->absfuzz, dev->absfuzz, size);
		memcpy(videv->absflat, dev->absflat, size);
	}
