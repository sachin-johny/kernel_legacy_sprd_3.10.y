...
#define RING_C_CROOM(ex_dev)	((ex_dev)->size - (ex_dev)->pos)
    ...
    /*
     * The vpipe driver has a "local" (invisible by peer driver)
...
ex_sysconf_trigger(ExDev* ex_dev)
{
     ...
     * accordingly. The "server" field says us
...
ex_dev_cleanup(ExDev* ex_dev)
{
     ...
     * The "server" field says us which
...
#ifdef TRACE_SYSCONF
    printk(VPIPE_MSG "ex_open for minor=%d is ok\n", minor);
#endif
    mutex_unlock(&ex_dev->lock);
...
ex_release (struct inode* inode, struct file* file)
{
	...
	ex_dev_cleanup(ex_dev);
#ifdef TRACE_SYSCONF
	printk(VPIPE_MSG "ex_release for minor=%d is ok\n", minor);
#endif
    }
     ...
     * writer side.
     *
     * It can return less then required bytes if writer side
     * is not ok.
     *
...
#ifdef TRACE_XIRQ
    printk(VPIPE_MSG "ex_read minor=%d count=%d\n", minor, count);
#endif
	/*
         * ensure only one thread executes this operation
	 */
...
#ifdef TRACE_XIRQ
    printk(VPIPE_MSG "ex_read from minor=%d completed done=%d\n",
		      minor, done);
#endif
...
#ifdef TRACE_XIRQ
    printk(VPIPE_MSG "ex_write minor=%d count=%d\n", minor, count);
#endif
	/*
         * ensure only one thread executes this operation
	 */
...
#ifdef TRACE_XIRQ
    printk(VPIPE_MSG "ex_write to minor=%d completed done=%d\n",
		      minor, done);
#endif
...
ex_dev_ring_alloc(ExDev* ex_dev)
{
    ...
    ex_dev->ring = nkops.nk_mem_map(pring, sz);
...
vpipe_module_init (void)
{
	...
	if ((vlink->s_id == my_id) || (vlink->c_id == my_id)) {
	    ...
	    if (ex_dev->enabled) {
		printk(VPIPE_INFO "device vpipe%d is created"
