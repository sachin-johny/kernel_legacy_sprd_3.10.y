...
#define RING_C_CROOM(ex_dev)	((ex_dev)->s_size - (ex_dev)->s_pos)
    ...
    /*
     * The vbpipe driver has a "local" (invisible by peer driver)
...
static struct class* ex_class;	/* "vbpipe" device class pointer */
     ...
     * The "server" parameter says us which ring is being reset.
     *
...
ex_dev_cleanup(ExDev* ex_dev)
{
     ...
     */
    static int
ex_xirq_attach (ExDev* ex_dev, NkXIrqHandler hdl, int server)
{
    NkDevVlink* link;
    NkXIrq      xirq;
    NkXIrqId	xid;
...
#ifdef TRACE_XIRQ
    printk(VBPIPE_MSG "ex_read minor=%d count=%d\n", minor, count);
#endif
	/*
         * ensure only one thread executes this operation
	 */
...
#ifdef TRACE_XIRQ
    printk(VBPIPE_MSG "ex_write minor=%d count=%d\n", minor, count);
#endif
	/*
         * ensure only one thread executes this operation
	 */
