 ...
 ****************************************************************
 *
 *  Component:	Red Bend Software VRTC-BE
...
#define ETRACE(x...)	printk (KERN_ERR     "VRTC-BE: " x)
...
vrtc_create (struct rtc_device* rtc_device, struct vrpc_t* vrpc)
{
    ...
    if (diag) {
	ETRACE ("Could not register RTC interrupt (%d)\n", diag);
	goto error_kfree;
    }
...
vrtc_init (void)
{
    ...
    vrtc_check ("rtc1");
#endif
    rtc_device = rtc_class_open ((char*) "rtc0");
    if (!rtc_device) {
	ETRACE ("No rtc0 device\n");
	return -ENODEV;
    }
