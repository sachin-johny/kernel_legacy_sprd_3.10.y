...
MODULE_AUTHOR("Guennadi Maslov<guennadi.maslov@virtuallogix.com> - VirtualLogix");
MODULE_LICENSE("GPL");
...
//#define TRACE_XIRQ		/* enable to see all cross interrupts */
     ...
     *
     * This circular buffer will mmapped by an application, then ioctl's
     * will be used to work with this buffer (send crossinterrupts
     ...
     * placed in the "shared" persistant memory. It is hidden from
...
#define RING_C_CROOM(ex_dev)	((ex_dev)->size - (ex_dev)->pos)
    ...
    /*
     * The vupipe driver has a "local" (invisible by peer driver)
...
typedef struct ExDev {
    ...
    int		server;		/* driver acts as a server */
    NkDevVlink* vlink;		/* vlink */
    ExRing*	ring;		/* circular ring descriptor */
    NkPhAddr	data;		/* circular ring data */
    int		size;		/* size of circular ring */
    int		pos;		/* reading/writing position inside ring */
     ...
     */
static int      ex_dev_num;	/* total number of communication links */
...
ex_sysconf_trigger(ExDev* ex_dev)
{
     ...
     * accordingly. The "server" parameter says us
...
ex_dev_cleanup(ExDev* ex_dev)
{
     ...
     * The "server" parameter says us which
...
#ifdef TRACE_SYSCONF
    printk(VUPIPE_MSG "ex_open for minor=%d is ok\n", minor);
#endif
    mutex_unlock(&ex_dev->lock);
...
ex_release (struct inode* inode, struct file* file)
{
	...
	ex_dev_cleanup(ex_dev);
#ifdef TRACE_SYSCONF
	printk(VUPIPE_MSG "ex_release for minor=%d is ok\n", minor);
#endif
    }
...
ex_read_notify(ExDev* ex_dev, unsigned long arg)
{
	...
	/*
	 * Reader cannot use more data then we have
	 */
...
ex_write_notify(ExDev* ex_dev, unsigned long arg)
{
	...
	/*
	 * Writer cannot use more room then we have
	 */
...
ex_available_data(ExDev* ex_dev, struct file* file, unsigned long arg)
{
	...
	/*
	 * Reader cannot use more data then buffer size
	 */
    if (arg > ex_dev->size) {
	return -EINVAL;
    }
	 ...
	 * retrun -EAGAIN if we have nothing;
	 ...
	 * This read operation will return less then required
...
ex_available_room(ExDev* ex_dev, struct file* file, unsigned long arg)
{
	...
	/*
	 * Writer cannot use more room then buffer size
	 */
    if (arg > ex_dev->size) {
	return -EINVAL;
    }
    ...
    /*
     * Main 6 ioctl's: read/write notify, available data/space
...
ex_dev_ring_alloc(ExDev* ex_dev)
{
    ...
    ex_dev->ring = nkops.nk_mem_map(pring, sizeof(ExRing));
    ex_dev->data = pdata;
     ...
     */
    static int
ex_dev_xirq_alloc(ExDev* ex_dev, int server)
{
    NkDevVlink* vlink = ex_dev->vlink;
    NkPhAddr    plink;
    NkXIrq      xirq;
     ...
     */
    static void
ex_dev_init (ExDev* ex_dev, int minor)
{
    NkDevVlink* vlink = ex_dev->vlink;
     ...
     */
    static void
ex_dev_destroy (ExDev* ex_dev, int minor)
{
	/*
