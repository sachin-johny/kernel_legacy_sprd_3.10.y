 ...
 ****************************************************************
 *
 *  Component:	Red Bend Software VRTC-FE
 *              Real Time Clock interface for VLX
 *
...
vrtc_op_proc (struct device* dev, struct seq_file* seq)
{
    ...
    static int
vrtc_op_irq_set_freq (struct device* dev, int freq)
{
    vrtc_t*	vrtc = dev_get_drvdata (dev);
    vrtc_res_t*	res = vrtc->vrpc_data;
    vrpc_size_t	size;
    ...
    size = vrtc_call (vrtc, VRTC_CMD_IRQ_SET_FREQ, freq);
    DTRACE ("got: size %d res %d value %d\n", size, res->res, res->value);
    if (size != sizeof (vrtc_res_t) || res->res) {
	return -EFAULT;
    }
    ...
    static int
vrtc_op_irq_set_state (struct device* dev, int enabled)
{
    vrtc_t*	vrtc = dev_get_drvdata (dev);
    vrtc_res_t*	res = vrtc->vrpc_data;
    vrpc_size_t	size;
    ...
    DTRACE ("enabled %d\n", enabled);
    size = vrtc_call (vrtc, VRTC_CMD_IRQ_SET_STATE, enabled);
    DTRACE ("got: size %d res %d value %d\n", size, res->res, res->value);
    if (size != sizeof (vrtc_res_t) || res->res) {
	return -EFAULT;
    }
...
vrtc_ops = {
    ...
    VRTC_ASSIGN (proc),
    .set_mmss = NULL,
    VRTC_ASSIGN (irq_set_state),
    VRTC_ASSIGN (irq_set_freq),
    .read_callback = NULL
};
