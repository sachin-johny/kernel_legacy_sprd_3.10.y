...
#if 0
    /*
     *  Declaring vbattery-fe as a platform driver does not cause
     *  the registered _vbat_probe() handler to be called on isolated
     *  2.6.32 for some reason, even when _vbat_init() is passed to
     *  late_initcall() rather than to module_init() (cf. vmtd-fe.c
     *  init code). So we just do not declare this driver as platform
     *  for now. Does not seem to serve any purpose, yet bloats code.
     */
...
#ifdef VBATTERY_DEBUG
static const char* vbat_cmd_name
		  [VBAT_CMD_MAX] =
		   VBAT_CMD_NAME;
static const char* vbat_power_supply_type_name
		  [VBAT_POWER_SUPPLY_TYPE_MAX] =
		   VBAT_POWER_SUPPLY_TYPE_NAME;
static const char* vbat_power_supply_status_name
		  [VBAT_POWER_SUPPLY_STATUS_MAX] =
		   VBAT_POWER_SUPPLY_STATUS_NAME;
static const char* vbat_power_supply_health_name
		  [VBAT_POWER_SUPPLY_HEALTH_MAX] =
		   VBAT_POWER_SUPPLY_HEALTH_NAME;
static const char* vbat_power_supply_technology_name
		  [VBAT_POWER_SUPPLY_TECHNOLOGY_MAX] =
		   VBAT_POWER_SUPPLY_TECHNOLOGY_NAME;
static const char* vbat_power_supply_property_name
