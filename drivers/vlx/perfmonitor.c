 ...
 *
 * #ident "@(#)osware-perfmon.c 1.5 06/07/25 Jaluna"
 *
 * Contributor(s):
...
#define ERR        "VirtualLogix VLX Performance Analysis: error -- "
...
MODULE_DESCRIPTION("VirtualLogix VLX Performance Analysis module");
MODULE_AUTHOR("Chi Dat TRUONG <chidat.truong@jaluna.com> - Jaluna SA");
...
_perfmon_init_data (void)
{
	...
	// Set version
	memcpy(pmon_sysinfo.version, PMON_VERSION_NUMBER, strlen(PMON_VERSION_NUMBER));
                                 ...
                                 loff_t*      ppos)
{
	...
	if (*ppos == 0) {
		...
		size += sprintf(sysinfo_tampon+size, "VirtualLogix VLX Performance Analysis v%s\n", pmon_sysinfo.version);
 ...
 */
	static int
_nk_perfmon_tree_create (struct proc_dir_entry* parent)
{
	int cpu;
	int error = 0;
 ...
 */
	static void
_nk_perfmon_tree_remove (struct proc_dir_entry* parent)
{
	int cpu;
	...
	for (cpu = 0 ; cpu < max_phys_cpus ; cpu++) {
		...
		/*
		 * Remove /proc/nk/cpu/# if it is empty
		 */
		if (! proc_data.cpunum[cpu]->subdir) {
	 ...
	 * Remove /proc/nk/cpu/# if it is empty
	 */
	if (! proc_data.cpu->subdir) {
		remove_proc_entry("cpu", parent);
	}
}
...
_perfmon_module_exit (void)
{
	...
	if (nk) {
		...
		*/
		_nk_perfmon_tree_remove(nk);
		/*
		 * Clean /proc/nk directory if it is empty
		 */
		if (!nk->subdir) {
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,26)
			remove_proc_entry("nk", &proc_root);
#else
...
_perfmon_module_init (void)
{
...
#endif
		if (!nk) {
			printk(ERR "proc_mkdir(/proc/nk) failed\n");
			return -1;
		}
	}

	if (_nk_perfmon_tree_create(nk) < 0) {
	...
	printk("\nVirtualLogix VLX Performance Analysis v%s\n", pmon_sysinfo.version);
	return 0;
}
