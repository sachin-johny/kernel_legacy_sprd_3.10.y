...
vlink_ops_cleanup (Vlink* vlink)
{
     ...
     */
    static inline void
vlink_admin_event_post (Vlink* vlink, unsigned int event)
{
    nkops.nk_atomic_set(&vlink->admin_event, event);
...
{
    wait_event_interruptible(vlink->admin_event_wait, vlink->admin_event != 0);
    return xchg(&vlink->admin_event, 0);
}
...
vlink_dump (Vlink* vlink)
{
    ...
    VLINK_LIB_DTRACE("Admin events .... %s\n",
		     vlink_admin_event_str(vlink->admin_event));
}
