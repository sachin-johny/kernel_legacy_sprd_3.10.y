...
#define NAND_MC_GETD(x)	(uint16_t)((NFC_MC_GETD_ID) | ((x -1) << 8))
...
uint8_t s_oob_data[NAND_MAX_OOBSIZE];
//static __attribute__((aligned(4))) uint8_t s_oob_data[8];
//static __attribute__((aligned(4))) uint8_t s_id_status[8];

STATIC_FUNC int sprd_dolphin_nand_read_id(struct sprd_dolphin_nand_info
					  *dolphin, uint32_t * buf);
STATIC_FUNC int sprd_dolphin_nand_reset(struct sprd_dolphin_nand_info *dolphin);
STATIC_FUNC int sprd_dolphin_nand_wait_finish(struct sprd_dolphin_nand_info
					      *dolphin);
					   ...
					   uint32_t nfc_clk_MHz)
{
	...
	reg_val |= ((temp_val & 0x3F) << NFC_RWL_OFFSET);
...
void boad_nand_param_init(struct sprd_dolphin_nand_info *dolphin,
			  struct nand_chip *chip, uint8 * id)
{
	int extid;
	uint32_t writesize;
	uint32_t oobsize;
	...
	dolphin->ecc_pos =
	    dolphin->s_size - ((14 * CONFIG_SYS_NAND_ECC_MODE + 7) / 8);
						  ...
						  struct nand_ecclayout *layout)
{
	...
	for (sct = 0; sct < sector_num; sct++) {
		...
		layout->oobfree[sct].offset =
		    oob->nOOBSize * sct + oob->nInfoPos;
		layout->oobfree[sct].length = oob->nInfoSize;
		oobfree_len += oob->nInfoSize;
	}

	layout->oobavail = oobfree_len;
			  ...
			  uint8_t id[5])
{
	...
	if (param != NULL) {
		...
		sprd_dolphin_nand_ecc_layout_gen(oob, dolphin->sct_pg,
						 &sprd_dolphin_nand_oob_default);
		chip->ecc.layout = &sprd_dolphin_nand_oob_default;

		mtd->writesize = dolphin->write_size;
		mtd->oobsize = dolphin->oob_size;
		mtd->erasesize = dolphin->page_per_bl * dolphin->write_size;
		/*bitflip_threshold be half of the ecc strength may be suitable */
		mtd->bitflip_threshold = (oob->nEccBits + 1) >> 1;
		...
		dolphin->info_pos =
		    dolphin->s_size - CONFIG_SYS_NAND_ECCBYTES - 1;
		dolphin->info_size = 1;
		...
		sprd_dolphin_nand_ecc_layout_gen(&oob_tmp, dolphin->sct_pg,
						 &sprd_dolphin_nand_oob_default);
...
STATIC_FUNC unsigned int sprd_dolphin_data_trans(sprd_dolphin_nand_data_param *
						 param)
{
	uint32_t cfg0 = 0;
	uint32_t cfg1 = 0;
	...
	if (param->m_sct != 0) {
		...
		cfg0 |= MAIN_USE;
		cfg1 |= (param->m_size - 1);
		sprd_dolphin_reg_write(NFC_MAIN_ADDR_REG,
				       (uint32_t) param->m_buf);
	}
	if (param->s_sct != 0) {
		cfg0 |= SPAR_USE;
		cfg1 |= (param->s_size - 1) << SPAR_SIZE_OFFSET;
		cfg2 |= (param->s_sct - 1) << SPAR_SECTOR_NUM_OFFSET;
		sprd_dolphin_reg_write(NFC_SPAR_ADDR_REG,
				       (uint32_t) param->s_buf);
	}
...
unsigned int sprd_ecc_encode(struct sprd_ecc_param *param)
{
	...
	Dcache_CleanRegion((unsigned int)d_param.m_buf,
			   d_param.m_sct * d_param.m_size);
	Dcache_CleanRegion((unsigned int)d_param.s_buf,
			   d_param.s_sct * d_param.s_size);
	...
	Dcache_InvalRegion((unsigned int)d_param.m_buf,
			   d_param.m_sct * d_param.m_size);
	Dcache_InvalRegion((unsigned int)d_param.s_buf,
			   d_param.s_sct * d_param.s_size);
...
STATIC_FUNC void sprd_dolphin_nand_ins_init(struct sprd_dolphin_nand_info
					    *dolphin)
{
	dolphin->ins_num = 0;
}

STATIC_FUNC void sprd_dolphin_nand_ins_add(uint16_t ins,
					   struct sprd_dolphin_nand_info
					   *dolphin)
{
...
STATIC_FUNC void sprd_dolphin_nand_ins_exec(struct sprd_dolphin_nand_info
					    *dolphin)
{
	uint32_t i;
	uint32_t cfg0;
	...
	for (i = 0; i < ((dolphin->ins_num + 1) >> 1); i++) {
		sprd_dolphin_reg_write(NFC_INST0_REG + (i << 2),
				       dolphin->ins[i]);
	}
	...
	cfg0 |= ((dolphin->chip << CS_SEL_OFFSET) & CS_SEL_MASK);
					      ...
					      *dolphin)
{
	...
	if (elapsed >= NFC_TIMEOUT_VAL) {
		printk(KERN_ERR
		       "sprd_dolphin_nand_wait_finish timeout %d usecs. Dump NandC Registers:  \n",
		       elapsed);
		print_hex_dump(KERN_ERR, "", DUMP_PREFIX_OFFSET, 32, 4,
			       SPRD_NFC_BASE, 0x140, 1);
		dump_stack();
		return -1;
	}
	return 0;
...
STATIC_FUNC void sprd_dolphin_nand_wp_en(struct sprd_dolphin_nand_info *dolphin,
					 int en)
{
	if (en) {
		dolphin->wp_en = 1;
...
STATIC_FUNC void sprd_dolphin_nand_read_status(struct sprd_dolphin_nand_info
					       *dolphin)
{
	uint32_t *buf;
	//DPRINT("%s enter\n", __func__);
	...
	sprd_dolphin_nand_ins_add(NAND_MC_GETD(1), dolphin);
					  ...
					  *dolphin, uint32_t * buf)
{
	...
	sprd_dolphin_nand_ins_add(NAND_MC_NOP(10), dolphin);
	sprd_dolphin_nand_ins_add(NAND_MC_GETD(8), dolphin);
	sprd_dolphin_nand_ins_add(NFC_MC_DONE_ID, dolphin);
...
#ifdef DOLPHIN_UBOOT
//read large page
STATIC_FUNC int sprd_dolphin_nand_read_lp(struct mtd_info *mtd, uint8_t * mbuf,
					  uint8_t * sbuf, uint32_t raw)
{
	...
	{
		page_addr >>= 8;
		sprd_dolphin_nand_ins_add(NAND_MC_ADDR(page_addr & 0xff),
					  dolphin);
	}
	...
	if (mbuf && sbuf) {
		...
		//switch to main part
		sprd_dolphin_nand_ins_add(NAND_MC_CMD(NAND_CMD_RNDOUT),
					  dolphin);
		sprd_dolphin_nand_ins_add(NAND_MC_ADDR(0), dolphin);
		sprd_dolphin_nand_ins_add(NAND_MC_ADDR(0), dolphin);
		sprd_dolphin_nand_ins_add(NAND_MC_CMD(NAND_CMD_RNDOUTSTART),
					  dolphin);
	...
	//config registers 
	cfg0 =
	    NFC_AUTO_MODE | MAIN_SPAR_APT | ((dolphin->sct_pg - 1) <<
					     SECTOR_NUM_OFFSET);
	if ((!raw) && mbuf && sbuf) {
	...
	cfg2 =
	    (dolphin->ecc_mode << ECC_MODE_OFFSET) | (dolphin->
						      info_pos <<
						      SPAR_INFO_POS_OFFSET) |
	    ((dolphin->sct_pg -
	      1) << SPAR_SECTOR_NUM_OFFSET) | dolphin->ecc_pos;

#ifndef CONFIG_NAND_SPL
	if (mbuf) {
		Dcache_CleanRegion((unsigned int)mbuf,
				   dolphin->m_size * dolphin->sct_pg);
		Dcache_InvalRegion((unsigned int)mbuf,
				   dolphin->m_size * dolphin->sct_pg);
	}

	if (sbuf) {
		Dcache_CleanRegion((unsigned int)sbuf,
				   dolphin->s_size * dolphin->sct_pg);
		Dcache_InvalRegion((unsigned int)sbuf,
				   dolphin->s_size * dolphin->sct_pg);
	}
#endif
	...
	if (mbuf && sbuf) {
		cfg1 |=
		    (dolphin->m_size -
		     1) | ((dolphin->s_size - 1) << SPAR_SIZE_OFFSET);
...
STATIC_FUNC int sprd_dolphin_nand_write_lp(struct mtd_info *mtd,
					   const uint8_t * mbuf, uint8_t * sbuf,
					   uint32_t raw)
{
	struct sprd_dolphin_nand_info *dolphin = mtd_to_dolphin(mtd);
	struct nand_chip *chip = dolphin->nand;
	...
	//config registers 
	cfg0 =
	    NFC_AUTO_MODE | NFC_RW | NFC_WPN | MAIN_SPAR_APT |
	    ((dolphin->sct_pg - 1) << SECTOR_NUM_OFFSET);
	if ((!raw) && mbuf && sbuf) {
		cfg0 |= ECC_EN;
	}
	if (chip->options & NAND_BUSWIDTH_16) {
...
#ifdef 	NAND_IRQ_EN
STATIC_FUNC void sprd_dolphin_nand_ins_exec_irq(struct sprd_dolphin_nand_info
						*dolphin)
{
	uint32_t i;
	uint32_t cfg0;
...
STATIC_FUNC int sprd_dolphin_nand_wait_finish_irq(struct sprd_dolphin_nand_info
						  *dolphin)
{
	unsigned int value;
...
STATIC_FUNC void nfc_wait_op_done(void)
{
	if (!wait_for_completion_timeout
	    (&nfc_op_completion, msecs_to_jiffies(IRQ_TIMEOUT))) {
					  ...
					  uint8_t * sbuf, uint32_t raw)
{
	...
	if (!raw) {
		for (i = 0; i < dolphin->sct_pg; i++) {
			...
			if(err == ERR_ERR_NUM0_MASK) {
			 ...
			 if(!ret) {
			 	if((ret_value >= 0) && (ecc_bit > ret_value))
					ret_value = ecc_bit;
			 }
			}
			else {
				mtd->ecc_stats.corrected += err;
				if ((ret_value >= 0) && (err > ret_value))
					ret_value = err;
			}
		}
	}
	...
	if (mbuf) {
		memcpy(mbuf, (const void *)dolphin->v_mbuf,
		       dolphin->write_size);
	}
	if (sbuf) {
	...
	return ret_value < 0 ? 0 : ret_value;
}
					   ...
					   uint32_t raw)
{
	...
	if (mbuf) {
		memcpy((void *)dolphin->v_mbuf, (const void *)mbuf,
		       dolphin->write_size);
	}
	if (sbuf) {
		memcpy((void *)dolphin->v_oob, (const void *)sbuf,
		       dolphin->oob_size);
	}
...
STATIC_FUNC void sprd_dolphin_erase(struct mtd_info *mtd, int page_addr)
{
	...
	page_addr >>= 8;
	sprd_dolphin_nand_ins_add(NAND_MC_ADDR(page_addr & 0xff), dolphin);
	if ((5 == dolphin->a_cycles)
	    || ((4 == dolphin->a_cycles) && (512 == dolphin->write_size))) {
		page_addr >>= 8;
		sprd_dolphin_nand_ins_add(NAND_MC_ADDR(page_addr & 0xff),
					  dolphin);
	}
...
/*We do the wait in all specific operation ,so we do not need wait timeout here ,
but the operation status need be return*/
STATIC_FUNC int sprd_dolphin_waitfunc(struct mtd_info *mtd,
...
STATIC_FUNC int sprd_dolphin_ecc_calculate(struct mtd_info *mtd,
					   const uint8_t * data,
					   uint8_t * ecc_code)
{
	return 0;
...
STATIC_FUNC int sprd_dolphin_read_page(struct mtd_info *mtd,
				       ...
				       int oob_required, int page)
{
	...
	if (512 == mtd->writesize) {
		ret = sprd_dolphin_nand_read_sp(mtd, buf, chip->oob_poi, 0);
	} else {
		ret = sprd_dolphin_nand_read_lp(mtd, buf, chip->oob_poi, 0);
	}
	return ret;
}

STATIC_FUNC int sprd_dolphin_read_page_raw(struct mtd_info *mtd,
					   struct nand_chip *chip,
					   ...
					   int page)
{
	...
	dolphin->page = page;
	if (512 == mtd->writesize) {
		ret = sprd_dolphin_nand_read_sp(mtd, buf, chip->oob_poi, 1);
	} else {
		ret = sprd_dolphin_nand_read_lp(mtd, buf, chip->oob_poi, 1);
	}
	return ret;
}

STATIC_FUNC int sprd_dolphin_read_oob(struct mtd_info *mtd,
				      ...
				      int sndcmd)
{
	...
	dolphin->page = page;
	if (512 == mtd->writesize) {
		ret = sprd_dolphin_nand_read_sp(mtd, 0, chip->oob_poi, 1);
	} else {
		ret = sprd_dolphin_nand_read_lp(mtd, 0, chip->oob_poi, 1);
	}
	return ret;
}

STATIC_FUNC int sprd_dolphin_write_page(struct mtd_info *mtd,
					struct nand_chip *chip,
					const uint8_t * buf, int oob_required)
{
	int ret = 0;
	if (512 == mtd->writesize) {
		ret = sprd_dolphin_nand_write_sp(mtd, buf, chip->oob_poi, 0);
	} else {
		ret = sprd_dolphin_nand_write_lp(mtd, buf, chip->oob_poi, 0);
	}
	return ret;
}

STATIC_FUNC int sprd_dolphin_write_page_raw(struct mtd_info *mtd,
					    struct nand_chip *chip,
					    const uint8_t * buf,
					    int oob_required)
{
	int ret = 0;
	if (512 == mtd->writesize) {
		ret = sprd_dolphin_nand_write_sp(mtd, buf, chip->oob_poi, 1);
	} else {
		ret = sprd_dolphin_nand_write_lp(mtd, buf, chip->oob_poi, 1);
	}
	return ret;
}

STATIC_FUNC int sprd_dolphin_write_oob(struct mtd_info *mtd,
				       struct nand_chip *chip, int page)
{
	struct sprd_dolphin_nand_info *dolphin = mtd_to_dolphin(mtd);
	int ret = 0;
	dolphin->page = page;
	if (512 == mtd->writesize) {
		ret = sprd_dolphin_nand_write_sp(mtd, 0, chip->oob_poi, 1);
	} else {
		ret = sprd_dolphin_nand_write_lp(mtd, 0, chip->oob_poi, 1);
	}
	return ret;
}
 ...
 */
STATIC_FUNC int sprd_dolphin_block_bad(struct mtd_info *mtd, loff_t ofs,
				       int getchip)
{
...
STATIC_FUNC void sprd_dolphin_nand_cmdfunc(struct mtd_info *mtd,
...
STATIC_FUNC void sprd_dolphin_nand_hw_init(struct sprd_dolphin_nand_info
					   *dolphin)
{
	uint32_t val;
	NAND_CLK_CONFG;
	...
	val =
	    (3) | (4 << NFC_RWH_OFFSET) | (3 << NFC_RWE_OFFSET) | (3 <<
								   NFC_RWS_OFFSET)
	    | (3 << NFC_ACE_OFFSET) | (3 << NFC_ACS_OFFSET);
...
static int sprd_scan_one_block(int blk, int erasesize, int writesize)
{
	...
	for (ii = 0; ii < 2; ii++) {
		...
		sprd_dolphin_nand_ins_add(NAND_MC_CMD(NAND_CMD_READ0),
					  &g_dolphin);
		sprd_dolphin_nand_ins_add(NAND_MC_CMD(NAND_CMD_READSTART),
					  &g_dolphin);
...
void nand_scan_patition(int blocks, int erasesize, int writesize)
{
	...
	for (blk = 0; blk < blocks; blk++) {
		...
		if (ret != 0) {
			...
			ret = nand_ctl_erase_block(blk, erasesize, writesize);
			DPRINT("erasing block : %d    %d % \r", blk,
			       (blk * 100) / blocks);
		}
	}
}
...
STATIC_FUNC int sprd_nand_dma_init(struct sprd_dolphin_nand_info *dolphin)
{
	...
	void *virt_ptr = 0;
	virt_ptr =
	    dma_alloc_coherent(NULL, dolphin->write_size, &phys_addr,
			       GFP_KERNEL);
	if (virt_ptr == NULL) {
		DPRINT(KERN_ERR
		       "NAND - Failed to allocate memory for DMA main buffer\n");
		return -ENOMEM;
	}
	...
	virt_ptr =
	    dma_alloc_coherent(NULL, dolphin->oob_size, &phys_addr, GFP_KERNEL);
	if (virt_ptr == NULL) {
		DPRINT(KERN_ERR
		       "NAND - Failed to allocate memory for DMA oob buffer\n");
		dma_free_coherent(NULL, dolphin->write_size,
				  (void *)dolphin->v_mbuf,
				  (dma_addr_t) dolphin->p_mbuf);
		return -ENOMEM;
	}
...
{
	dma_free_coherent(NULL, dolphin->write_size, (void *)dolphin->v_mbuf,
			  (dma_addr_t) dolphin->p_mbuf);
	dma_free_coherent(NULL, dolphin->write_size, (void *)dolphin->v_oob,
			  (dma_addr_t) dolphin->p_oob);
}
...
STATIC_FUNC int sprd_nand_probe(struct platform_device *pdev)
{
	...
	int ret = 0;
	struct regulator *mmc_regu = NULL;
	...
	if (sprd_dolphin_nand_read_id(&g_dolphin, (uint32_t *) s_oob_data) != 0) {
		ret = -ENXIO;
		DPRINT(KERN_ERR
		       "nand read id failed, no nand device!!!!!!!!!!!!!\n");
	} else
		DPRINT(KERN_ALERT
		       "nand read id ok, nand exists!!!!!!!!!!!!!\n");
	...
	num_partitions =
	    parse_mtd_partitions(sprd_mtd, part_probes, &partitions, 0);
