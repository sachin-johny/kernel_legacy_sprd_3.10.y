...
#define PRINT_PREF KERN_INFO "mtd_oobtestread: "
...
static int verify_eraseblock(int ebnum)
{
	...
	for (i = 0; i < pgcnt; ++i, addr += mtd->writesize) {
		set_random_data(writebuf, use_len);
		/*for (aaa = 0; aaa < use_len; aaa++)
			printk("writebuf2[%d]=0x%02x  ", aaa, writebuf[aaa]);
		...
		/*for (aaa = 0; aaa < use_len; aaa++)
			printk("readbuf2[%d]=0x%02x  ", aaa, readbuf[aaa]);*/
...
static int verify_eraseblock_in_one_go(int ebnum)
{
	...
	for (aaa = 0; aaa < len; aaa++)
		printk("writebuf3[%d]=0x%02x  ", aaa, writebuf[aaa]);*/
	ops.mode      = MTD_OOB_AUTO;
	ops.len       = 0;
	ops.retlen    = 0;
	ops.ooblen    = len;
	...
	ops.oobbuf    = readbuf;
	err = mtd->read_oob(mtd, addr, &ops);
	//printk("err=%d  len=%d  oobretlen=%d\n", err, len, ops.oobretlen);
	...
	/*for (aaa = 0; aaa < len; aaa++)
		printk("readbuf3[%d]=0x%02x  ", aaa, readbuf[aaa]);*/
...
static int __init mtd_oobtestread_init(void)
{
	int err = 0;
	unsigned int i;
	uint64_t tmp;
...
module_init(mtd_oobtestread_init);

static void __exit mtd_oobtestread_exit(void)
{
	return;
}
module_exit(mtd_oobtestread_exit);
