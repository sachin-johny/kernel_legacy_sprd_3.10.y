...
int main(int argc, char * const argv[])
{
...
#if 1
        struct mtd_pageoob_buf pageoob;
#endif
		...
		&& (mtdoffset < meminfo.size))
	{
		...
		if (writeoob) {
			...
			oob.start = mtdoffset;
#if 0
			/* oob will be written with page data */
...
#if 1
                pageoob.oobptr = oob.ptr;
                pageoob.datptr = writebuf;
                pageoob.start = oob.start;
		if (autoplace) {
			pageoob.ooblength = meminfo.oobsize - old_oobinfo.eccbytes;
			printf("oobsize=%d eccbytes=%d\n", meminfo.oobsize, old_oobinfo.eccbytes);
		} else {
			pageoob.ooblength = meminfo.oobsize - autoplace_oobinfo.eccbytes;
			printf("oobsize=%d eccbytes=%d\n", meminfo.oobsize, autoplace_oobinfo.eccbytes);
		}
                /* Write out the Page data and oob data */
                ioctl(fd, MEMWRITEPAGEOOB, &pageoob);
		printf("datlength = %d\n", pageoob.datlength);
                if (pageoob.datlength != meminfo.writesize) {
#else
