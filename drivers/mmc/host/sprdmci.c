...
/* SDHCI_BUS_SCAN may need to be defined for sdio devices, such as wifi */
#define SDHCI_BUS_SCAN
/* MMC_AUTO_SUSPEND should not be defined for EMMC, buf T-flash card */
...
#define HOST_WAKEUP_IRQ_EN   BIT_6
#endif

#define DRIVER_NAME  "sprdmci"
...
/*
 * TODO:
 * wakeup source is not ready yet, keep comment here for debug
 */
void sdhci_set_data1_to_gpio(struct sdhci_host *host)
{
	...
	static unsigned long sdiod1_gpio_func_cfg[] = {
		/* gpio, input */
		MFP_CFG_X(SD2_D1, AF3, DS1, F_PULL_UP, S_PULL_UP, IO_IE),
	};
	...
	val = BITS_PIN_DS(1) | BITS_PIN_AF(3)  |
		...
		BIT_PIN_SLP_IE ;
	__raw_writel( val, REG_PIN_SD2_D1 );
...
static void  sdhci_host_wakeup_set( struct wake_source *src )
{
	...
	if( (host->mmc->card )&& mmc_card_sdio(host->mmc->card) ){
		val = __raw_readl(REG_GPIO_IE);
		val |= HOST_WAKEUP_IRQ_EN;
		__raw_writel(val, REG_GPIO_IE); /* gpio22 irq enable */
		printk("set: REG_GPIO_IE:0x%x\n", __raw_readl( REG_GPIO_IE) );
	}
#endif
	return;
}
...
{
	pr_debug("%s, %s power:%x ---> power:%x\n", __func__,
			mmc_hostname(host->mmc), host->pwr, power);
	u8 pwr;
...
static void sdhci_tasklet_card(unsigned long param)
{
	...
	spin_lock_irqsave(&host->lock, flags);
	/* hot-plug is not supported in our host, we use gpio */
...
static void sdhci_tasklet_finish(unsigned long param)
{
	...
	if(mrq){
		if( !(host->flags & SDHCI_DEVICE_DEAD) &&
		( mrq->cmd->error || (mrq->data && (mrq->data->error ||
		...
		(host->quirks & SDHCI_QUIRK_RESET_AFTER_REQUEST) ) ){
			...
			/*
			 *  It's  a 8800G IC bug.
			 *  if host issue a command error interrupt SDHCI_INT_TIMEOUT,
			 *after reset CMD,host will issue another interrupt
			 *SDHCI_INT_RESPONSE which should never come.so we delay a while
			 *and check the interrupt status, and remove it if we find it.
			 *so with the data error SDHCI_INT_DATA_TIMEOUT. After reset  DATA,
			 *host will issue SDHCI_INT_DATA_END, and we check  and remove it too.
			 **/
			if (mrq->cmd && (mrq->cmd->error == -ETIMEDOUT)) {
				u32 count = WEIRD_TIMEOUT;
				while( !(sdhci_readl(host, SDHCI_INT_STATUS) &
					SDHCI_INT_RESPONSE) && count ){
					udelay(1);
					count--;
				}
...
int sdhci_suspend_host(struct sdhci_host *host, pm_message_t state)
{
	...
	if( (host->mmc->card )&& mmc_card_sdio(host->mmc->card) ){
		...
		request_threaded_irq(sdio_wakeup_irq, sdhci_wakeup_irq_handler, NULL,
...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	if (caps[0] & SDHCI_CAN_VDD_330) {
		...
		max_current_330 = ((max_current_caps & SDHCI_MAX_CURRENT_330_MASK) >>
...
#ifdef MMC_AUTO_SUSPEND
	setup_timer(&host->auto_suspend_timer, sdhci_auto_suspend_host,
							(unsigned long)host);
#endif
...
MODULE_DESCRIPTION("Spreadtrum Secure Digital Host Controller Interface core driver");
MODULE_LICENSE("GPL");
