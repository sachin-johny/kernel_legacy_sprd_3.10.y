...
static void sdhci_sprd_set_power(struct sdhci_host *host, unsigned int power)
{
	...
	if(volt_level == 0){
		...
		if(host->vqmmc){
			ret = regulator_set_voltage(host->vqmmc, volt_ext_level,
								volt_ext_level);
			if(ret){
				printk(KERN_ERR "%s, vqmmc set voltage error:%d\n",
					mmc_hostname(host->mmc), ret);
				return;
			}
			ret = regulator_enable(host->vqmmc);
			if(ret){
				printk(KERN_ERR "%s, vqmmc enabel regulator error:%d\n",
					mmc_hostname(host->mmc), ret);
				return;
			}
		}
...
static int sdhci_sprd_probe(struct platform_device *pdev)
{
	...
	if(host_data->platdata->vdd_ext_name) {
	    host->vqmmc = regulator_get(NULL, host_data->platdata->vdd_ext_name);
