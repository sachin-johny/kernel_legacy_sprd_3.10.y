 ...
 *
 * SC8800S LCDC framebuffer driver.
 *
...
/* gpio id */
#define GPIO_PROD_LCD_BL_BRIGHTNESS_ID 83
#define GPIO_PROD_LCD_RST              102
...
int sc8800sfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	*/
	...
	__raw_writel(reg_val, LCDC_LCMDATANUMBER);
	...
	while((__raw_readl(LCDC_LCM_INT_STATUS) & 1) == 0);
	/* clear bit */
	__raw_bits_or((1<<0), LCDC_LCM_INT_STATUS);
...
static int32_t lcm_send_cmd (uint32_t cmd)
{
	...
	while(__raw_readl(LCDC_LCMSTATUS) & 0x1);

	__raw_writel(cmd, LCDC_LCM_CS0_CD0_CMD);
...
static int32_t lcm_send_cmd_data (uint32_t cmd, uint32_t data)
{
	...
	__raw_writel(data, LCDC_LCM_CS0_CD0_DATA);
...
static irqreturn_t lcdc_isr(int irq, void *data)
{
	...
	if((__raw_readl(LCDC_LCD_MODE) & (1<<1)) != 1) { /* !TV int */
		...
		uint32_t val = __raw_readl(LCDC_LCM_INT_VECTOR);

		if (val & (1<<1)) { /* fifo overflow */
			__raw_bits_or((1<<1), LCDC_LCM_INT_STATUS);
	FB_PRINT("@fool2[%s] fifo overflow\n", __FUNCTION__);
		}
		if (val & (1<<0)) { /* OSD frame done */
...
static void lcdc_mcu_init(void)
{
	...
	__raw_bits_or((1<<11), LCDC_LCD_MODE); /* select LCM1 */
	__raw_bits_or((1<<10), LCDC_LCD_MODE); /* enable LCM1 */
	mdelay(1);
	__raw_writel(1, LCDC_LCMRSTN);
	__raw_bits_or((1<<2),  LCDC_LCD_MODE); /* enable lcdc */

	__raw_writel(0xff, LCDC_LCD_BACKGROUND); /* set background */

	__raw_bits_or((1<<1), LCDC_LCM_INT_EN); /* enable overflow int */
...
static void lcdc_mcu_configure(struct sc8800sfb_info *sc8800sfb)
{
	...
	__raw_writel(0, LCDC_LCMPARAMETER0); /* LCM_PARAMETER0 */
	...
	__raw_writel((bits&0x1), LCDC_LCMMODESEL);
	FB_PRINT("@fool2[%s] LCDC_LCMMODESEL: 0x%x\n", __FUNCTION__, __raw_readl(LCDC_LCMMODESEL));
	...
	__raw_writel((bits&0x3), LCDC_LCM_RGB_MODE);
	FB_PRINT("@fool2[%s] LCDC_LCM_RGB_MODE: 0x%x\n", __FUNCTION__, __raw_readl(LCDC_LCM_RGB_MODE));

	__raw_writel(0, LCDC_LCM_SELPIN);
	FB_PRINT("@fool2[%s] LCDC_LCM_SELPIN: 0x%x\n", __FUNCTION__, __raw_readl(LCDC_LCM_SELPIN));
}
...
static void lcd_update_timing(struct sc8800sfb_info *sc8800sfb)
{
	...
	FB_PRINT("@fool2[%s] LCDC_LCMPARAMETER0: 0x%x\n", __FUNCTION__, __raw_readl(LCDC_LCMPARAMETER0));
}
...
static void hw_init(struct sc8800sfb_info *sc8800sfb)
{
	...
	/* register isr */
	ret = request_irq(IRQ_LCDC_INT, lcdc_isr, IRQF_DISABLED, 
		"LCDC", sc8800sfb);
	if (ret) {
...
static void set_pins(void)
{
	...
	__raw_writel(1|0x10, (SPRD_CPC_BASE+0x1f8));
	__raw_writel(1|0x10, (SPRD_CPC_BASE+0x200));
	...
	__raw_writel(0x10, (SPRD_CPC_BASE+0x248));   /* GPIO103 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x24c));   /* GPIO104 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x250));   /* GPIO105 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x254));   /* GPIO106 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x258));   /* GPIO107 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x25c));   /* GPIO108 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x260));   /* GPIO109 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x264));   /* GPIO110 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x268));   /* GPIO111 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x26c));   /* GPIO112 */
	__raw_writel(1|0x10, (SPRD_CPC_BASE+0x270));   /* GPIO113 */
	__raw_writel(1|0x10, (SPRD_CPC_BASE+0x274));   /* GPIO114 */
	__raw_writel(0x10|0x40, (SPRD_CPC_BASE+0x278));   /* GPIO115 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x338));   /* GPIO116 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x33c));   /* GPIO117 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x340));   /* GPIO118 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x344));   /* GPIO119 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x348));   /* GPIO120 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x34c));   /* GPIO121 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x350));   /* GPIO122 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x354));   /* GPIO123 */
	__raw_writel(0x10, (SPRD_CPC_BASE+0x358));   /* GPIO124 */
...
{
	/* LDO Core, 1.65mv */
	__raw_writel((1<<8)|0x3, SPRD_GREG_BASE+0x80);
	__raw_bits_and(~(1<<8), SPRD_GREG_BASE+0x80);
