...
static int sec_bat_get_charger_type_adc(struct sec_battery_info *battery)
{
		...
		int cable_source_type)
{
	...
	if (cable_source_type & SEC_BATTERY_CABLE_SOURCE_CALLBACK) {
		cable_type = battery->pdata->check_cable_callback(battery);
	}
		...
		struct sec_battery_info *battery)
{
	...
	pdata->polling_time = devm_kzalloc(dev, sizeof(*pdata->polling_time) * len, GFP_KERNEL);
...
static int sec_battery_probe(struct platform_device *pdev)
{
	...
	if (battery->pdata->check_batt_id)
		battery->pdata->check_batt_id(battery);
