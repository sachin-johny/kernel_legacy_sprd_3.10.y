...
unsigned int sprdfgu_irq = IRQ_ANA_FGU_INT;
...
static int FGU_IMPEDANCE =  215;	//21.5moh

static int FGU_IMPEDANCE_IDEA = 200;	//200
static int  SPRDFGU_BATTERY_WARNING_VOL = 3500;
...
static int sprdfgu_full_capacity = 1500;
static int capacity_from_vol;
...
#endif
#endif
static int relax_curr;

static uint16_t  (* capacity_table)[2];
...
static uint32_t sprdfgu_vol2capacity(uint32_t voltage)
{
	...
	table_size = SPRDFGU_OCV_TAB_LEN;//ARRAY_SIZE(capacity_table);
...
static int sprdfgu_cal_init(void)
{
	vol_1000mv_adc = ((fgu_nv_4200mv) * 10 + 21) / 42;
	vol_offset = 0;
	cur_offset = 0;
	cur_1000ma_adc =
	    (vol_1000mv_adc * 4 * FGU_IMPEDANCE +
...
static int sprdfgu_int_init(void)
{
	...
	ret = request_irq(sprdfgu_irq, _sprdfgu_interrupt,
...
static void sprdfgu_hw_init(void)
{
	...
	sci_adi_write(REG_FGU_RELAX_CURT_THRE, BITS_RELAX_CUR_THRE(sprdfgu_cur2adc_ma(relax_curr)), BITS_RELAX_CUR_THRE(~0));	//mingwei
	udelay(130);
	...
	if ((FIRST_POWERTON == sprdfgu_poweron_type_read())
		...
		{
			int poci = sprdfgu_reg_get(REG_FGU_CLBCNT_QMAXL) << 1;
			uint32_t p_ocv = sprdfgu_adc2vol_mv(pocv_raw) -
			    (sprdfgu_adc2cur_ma
			     (poci - CUR_0ma_IDEA_ADC +
			      cur_offset) * battery_internal_impedance) / 1000;
			pcapacity = sprdfgu_vol2capacity(p_ocv);
			FGU_DEBUG("poci:%d,p_ocv:%d,pcapacity:%d\n", poci,
				  p_ocv, pcapacity);
		}
...
bool static sprdfgu_param_init(fuelgauge_variable_t * fg_var)
{
	struct battery_data_t * p_bat_param = &get_battery_data(fg_var) ;
	capacity_from_vol = p_bat_param->vmode;
	if (capacity_from_vol) {
		FGU_DEBUG("voltage_capacity_table  \n");
		capacity_table = voltage_capacity_table;
	}else{
		capacity_table = p_bat_param->ocv_table;
		FGU_DEBUG("p_bat_param->ocv_table  \n");
		{
			int i = 0;
			for (i = 0; i < SPRDFGU_OCV_TAB_LEN; i++) {
				FGU_DEBUG("init table i = %d cap=%d: vol=%d\n",
					  i, capacity_table[i][0],
					  capacity_table[i][1]);
			}
		}
	}
	relax_curr = p_bat_param->relax_current;
	sprdfgu_irq = p_bat_param->fgu_irq;
	sprdfgu_full_capacity = p_bat_param->cnom;
	FGU_DEBUG("sprdfgu_param_init cnom = %d \n",p_bat_param->cnom);

	SPRDFGU_BATTERY_WARNING_VOL = p_bat_param->alm_vbat;
	battery_internal_impedance = p_bat_param->rint;
	FGU_DEBUG("sprdfgu_param_init rint = %d \n",p_bat_param->rint);

	FGU_IMPEDANCE_IDEA = p_bat_param->rsense_spec;
	FGU_IMPEDANCE = p_bat_param->rsense_real;
	FGU_DEBUG("sprdfgu_param_init rsense_real = %d \n",
		  p_bat_param->rsense_real);
	return true;
}
...
static uint32_t sprdbat_update_capacty(void)
{
	...
	sprdfgu_rtc_reg_write(update_capacity);
