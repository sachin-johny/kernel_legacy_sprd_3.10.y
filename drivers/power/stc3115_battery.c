...
#if defined(CONFIG_SPA)
static struct stc311x_chip *spa_sta311x_chip;
#endif
...
//Check battery presence
  ...
  GG_Ram.reg.SOC = (GG->SOC+8)/10;    /* trace SOC in % */
  UpdateRamCrc();
  STC311x_WriteRamData(GG_Ram.db);
...
static void stc311x_work(struct work_struct *work)
{
    ...
    {
        /* results available */
        chip->batt_soc = (GasGaugeData.SOC+10)/10;
...
static int read_voltage(int *vbat)
{
	...
	printk("read_voltage enter\n");
	chip = spa_sta311x_chip;
