...
#include <mach/gpio.h>
#include <linux/headset-shark-temp.h>
...
static struct _headset_gpio *hgp_now = NULL;
static int irq_now = 0;
static int active_glb;
...
static unsigned int headset_map_code2key_type(unsigned int code)
{
	...
	if (i < ARRY_SIZE(headset_key_capability) &&
		...
		PRINT_ERR("headset not find code [0x%x]'s maping type\n", code);
	return key_type;
}
...
static enum hrtimer_restart report_headset_button_status(int active, struct _headset_gpio *hgp)
{
	...
	if (button_push_type != HEADSET_BUTTON_DOWN_INVALID) {
		...
		case EV_KEY:
			...
			input_sync(hgp->parent->input);
			PRINT_INFO("headset button-%d[%dms]\n", code, hgp->holded);
			break;
		default:
			PRINT_ERR("headset not support key type [%d]\n", key_type);
		}
	}
	return restart;
}
...
static enum hrtimer_restart headset_gpio_timer_func(struct hrtimer *timer)
{
	...
	if (active != hgp->active) {
		PRINT_INFO("The value %s mismatch [%d:%d] at %dms!\n",
				hgp->desc, active, hgp->active, hgp->holded);
		hgp->holded = 0;
	}
	PRINT_DBG("%s : %s %s green_ch[%d], holed=%d, debounce_sw=%d\n", __func__,
	...
	if (hgp->holded >= hgp->debounce_sw || green_ch) {
		if (hgp->holded == hgp->debounce_sw || \
			...
			green_ch) {
			PRINT_DBG("call headset gpio handler\n");
			restart = hgp->callback(active, hgp);
		} else
			PRINT_DBG("gpio <%d> has kept active for %d ms\n", hgp->gpio, hgp->holded);
	}
...
static irqreturn_t headset_gpio_irq_handler(int irq, void *dev)
{
	...
	irq_now = irq;
	ENTER;
	disable_irq_nosync(irq_now);
	queue_work(headset.check_sts_work_queue, &headset.check_sts_work);
	return IRQ_HANDLED;
#if 0
...
static void headset_switch_state(struct work_struct *work)
{
	...
	PRINT_INFO("set headset state to %d\n", type);
}

static void check_sts_state_func(struct work_struct *work)
{
	struct _headset *ht;
	int ana_sts0 = 0;
	int ana_cfg0 = 0;
	ENTER;
	ht = container_of(work, struct _headset, check_sts_work);

	if(irq_now == headset.detect.irq) {
		sci_adi_write(ANA_REG_GLB_ARM_MODULE_EN, BIT_ANA_AUD_EN, BIT_ANA_AUD_EN);
		ana_sts0 = sci_adi_read(ANA_AUDCFGA_INT_BASE+0xC0);
		ana_cfg0 = sci_adi_read(ANA_AUDCFGA_INT_BASE+0x40);
		PRINT_DBG("irq occured! [detect irq (%d)]  ana_sts0(0x%08X) = 0x%08X", irq_now, ANA_AUDCFGA_INT_BASE+0xC0, ana_sts0);
		PRINT_DBG("irq occured! [detect irq (%d)]  ana_cfg0(0x%08X) = 0x%08X", irq_now, ANA_AUDCFGA_INT_BASE+0x40, ana_cfg0);
		if(!((0x60 == (ana_sts0 & 0x00000060))  || (active_glb == 1)) ){
			msleep(1000);
			enable_irq(irq_now);
			hgp_now = NULL;
			irq_now = 0;
	...
	if(irq_now == headset.button.irq)
		PRINT_DBG("irq occured! [button irq (%d)]", irq_now);
	hrtimer_cancel(&hgp_now->timer);
	hgp_now->active = hgp_now->active_low ^ headset_gpio_get_value(hgp_now->gpio);
	headset_gpio_set_irq_type(hgp_now->irq, hgp_now->active ? hgp_now->irq_type_inactive : hgp_now->irq_type_active);
	PRINT_DBG("%s : %s %s\n", __func__, hgp_now->desc, hgp_now->active ? "active" : "inactive");
	hgp_now->holded = 0;
	hrtimer_start(&hgp_now->timer,
			ktime_set(HEADSET_GPIO_DEBOUNCE_SW_SAMPLE_PERIOD / 1000,
				(HEADSET_GPIO_DEBOUNCE_SW_SAMPLE_PERIOD % 1000) * 1000000),
			HRTIMER_MODE_REL);
...
static int __init headset_init(void)
{
...
#if 0
	PRINT_DBG("ANA_REG_GLB_ARM_MODULE_EN = 0x%08X\n", sci_adi_read(ANA_REG_GLB_ARM_MODULE_EN));
	ret = sci_adi_write(ANA_REG_GLB_ARM_MODULE_EN, BIT_ANA_AUD_EN, BIT_ANA_AUD_EN);
	PRINT_DBG("ANA_REG_GLB_ARM_MODULE_EN = 0x%08X\n", sci_adi_read(ANA_REG_GLB_ARM_MODULE_EN));

	PRINT_DBG("ANA_CFG0 = 0x%08X\n", sci_adi_read(ANA_AUDCFGA_INT_BASE+0x40));
	ret = sci_adi_write(ANA_AUDCFGA_INT_BASE+0x40, BIT(7), BIT(7));
	ret = sci_adi_write(ANA_AUDCFGA_INT_BASE+0x40, BIT(6), BIT(6));
	ret = sci_adi_write(ANA_AUDCFGA_INT_BASE+0x40, BIT(5), BIT(5));
	ret = sci_adi_write(ANA_AUDCFGA_INT_BASE+0x40, BIT(4), BIT(4));
	ret = sci_adi_write(ANA_AUDCFGA_INT_BASE+0x40, BIT(3), BIT(3));
	ret = sci_adi_write(ANA_AUDCFGA_INT_BASE+0x40, BIT(2), BIT(2));
	ret = sci_adi_write(ANA_AUDCFGA_INT_BASE+0x40, BIT(1), BIT(1));
	PRINT_DBG("ANA_CFG0 = 0x%08X\n", sci_adi_read(ANA_AUDCFGA_INT_BASE+0x40));
	...
	if (ret) {
		PRINT_ERR("request_irq gpio %d's irq failed!\n", ht->button.gpio);
		goto _gpio_request;
	}
	headset_gpio_irq_enable(0, &ht->button);
	...
	INIT_WORK(&ht->check_sts_work, check_sts_state_func);
	ht->check_sts_work_queue = create_singlethread_workqueue("yasin-lee");
	if (!ht->check_sts_work_queue) {
			PRINT_ERR("create_singlethread_workqueue failed!\n");
	}

	PRINT_INFO("ht->detect.irq=%d\n", ht->detect.irq);
	PRINT_INFO("ht->detect.gpio=%d\n", ht->detect.gpio);
	...
	if (ht->switch_workqueue == NULL) {
		PRINT_ERR("can't create headset switch workqueue\n");
		ret = -ENOMEM;
		goto _headset_button_gpio_irq_handler;
	}
