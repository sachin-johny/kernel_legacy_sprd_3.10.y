...
static void ltr558_work(struct work_struct *work)
{
	...
	if (0x03 == (als_ps_status & 0x03))
	{
		...
		if (val >= 0x250) { // 3cm
			...
			ltr558_i2c_write_reg(0x92, 0x40);
			ltr558_i2c_write_reg(0x93, 0x02);

			input_report_abs(pls->input, ABS_DISTANCE, 0);
			input_sync(pls->input);
		} else if (val <= 0x240) { // 5cm
			ltr558_i2c_write_reg(0x90, 0x50);        
			ltr558_i2c_write_reg(0x91, 0x02);
...
static int ltr558_reg_init(void)
{
	...
	// als
	ltr558_i2c_write_reg(0x97, 0x01);
	ltr558_i2c_write_reg(0x98, 0x00);
	ltr558_i2c_write_reg(0x99, 0x00);
	...
	printk("%s: 0x90 = [%x], 0x91 = [%x]\n",__func__,  ltr558_i2c_read_reg(0x90), ltr558_i2c_read_reg(0x91));
	printk("%s: 0x92 = [%x], 0x93 = [%x]\n", __func__, ltr558_i2c_read_reg(0x92), ltr558_i2c_read_reg(0x93));
	printk("%s: 0x97 = [%x], 0x98 = [%x]\n", __func__, ltr558_i2c_read_reg(0x97), ltr558_i2c_read_reg(0x98));
	printk("%s: 0x99 = [%x], 0x9a = [%x]\n",__func__,  ltr558_i2c_read_reg(0x99), ltr558_i2c_read_reg(0x9a));
#endif
...
static int ltr558_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!ltr_558als->ltr_work_queue) {
		alert_mesg("call create_singlethread_workqueue() error, ret = %d\n", ret);
