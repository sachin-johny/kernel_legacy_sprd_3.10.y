...
static int sy_rxdata(struct touch_info *cinfo, struct touch_info *pinfo)
{
	...
	IicReadData(SLAVE_I2C_ID_DWI2C, reg_val, 8);
	...
	if((temp_checksum==reg_val[7])&&(reg_val[0] == 0x52))
	{
        if ((reg_val[1] == 0xFF) && (reg_val[2] == 0xFF) && (reg_val[3] == 0xFF) && (reg_val[4] == 0xFF) && (reg_val[6] == 0xFF))
        {
		 ...
		 if(keycode==1)
		 {
		      cinfo->x1 = g_TpVirtualKeyDimLocal[0][0]; // final X coordinate
		      cinfo->y1 = g_TpVirtualKeyDimLocal[0][1]; // final Y coordinate
		 }
		 else if(keycode==2)
		 {
		      cinfo->x1 = g_TpVirtualKeyDimLocal[2][0]; // final X coordinate
		      cinfo->y1 = g_TpVirtualKeyDimLocal[2][1]; // final Y coordinate
		 }
		 else if (keycode==4)
		 {
		      cinfo->x1 = g_TpVirtualKeyDimLocal[1][0]; // final X coordinate
		      cinfo->y1 = g_TpVirtualKeyDimLocal[1][1]; // final Y coordinate
		 }
		 cinfo->count  = 1;
            }
        }
	else
	{
		touchData.nTouchKeyMode = 0; //Touch on screen...
			...
			if ((dst_x == 0) && (dst_y == 0))
			{
				...
				cinfo->x1 = (cinfo->x1 * 480) / 2048;
				cinfo->y1 = (cinfo->y1 * 800) / 2048;
			} else {
				...
				cinfo->x2 = (cinfo->x2 * 480) / 2048;
				cinfo->y2 = (cinfo->y2 * 800) / 2048;
			}
		}
        return 1;
    }
			...
			if (point_slot[i].active == 1) {
				if(point_slot[i].posy<0) {
					...
					if(point_slot[i].posx<0) {
						point_slot[i].posx = 0;
					} else if (point_slot[i].posx>(480 -2)) {
						point_slot[i].posx = (800 -2);
					}
					input_report_abs(g_InputDevice, ABS_MT_POSITION_X,  point_slot[i].posx);
					input_report_abs(g_InputDevice, ABS_MT_POSITION_Y,  point_slot[i].posy);
					input_report_key(g_InputDevice, BTN_TOUCH, 1);
					input_mt_sync(g_InputDevice);
					DBG("%s: slot=%d,x%d=%d,y%d=%d\n",__func__, i, i/2,point_slot[i].posx, i/2, point_slot[i].posy);
				}
			}
		}
	} else {
...
#endif //DrvFwCtrlHandleFingerTouch
