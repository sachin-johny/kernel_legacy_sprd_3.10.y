...
struct regulator *tsp_regulator_30 = NULL;
...
void touch_ctrl_regulator(int on_off)
{
	...
	{
        printk("Tsp off\n");
		regulator_disable(tsp_regulator_30);
//      udelay(100);
//		regulator_disable(tsp_regulator_18);
    }
    else
    {
	    regulator_enable(tsp_regulator_30);
//      udelay(100);
...
static int tsp_regulator(void)
{
	...
	tsp_regulator_30 = regulator_get(NULL, REGU_NAME_TP);
	if (IS_ERR(tsp_regulator_30)) {
		pr_err("zinitix:could not get 3.3v regulator\n");
		return -1;
	}
	err =regulator_set_voltage(tsp_regulator_30,3000000,3000000);
	if (err)
...
static void __exit tsp_driver_exit(void)
{
	if (tsp_regulator_30) 
	{
       	 regulator_put(tsp_regulator_30);
		 tsp_regulator_30 = NULL;
   	}
#if defined (__TOUCH_KEYLED__)
