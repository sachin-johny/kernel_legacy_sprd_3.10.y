...
// Add for TP proximity support; by kevin 2013-11-24
...
static int debug_level=1;//paul 0->1
...
static ssize_t virtual_keys_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	return sprintf(buf,
         __stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":60:520:10:10"
	 ":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE)   ":160:520:10:10"
	 ":" __stringify(EV_KEY) ":" __stringify(KEY_BACK) ":260:520:10:10"
	 "\n");
#else
	return sprintf(buf,
         __stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":80:855:10:10"
	 ":" __stringify(EV_KEY) ":" __stringify(KEY_HOMEPAGE)   ":240:855:10:10"
	 ":" __stringify(EV_KEY) ":" __stringify(KEY_BACK) ":400:855:10:10"
	 "\n");
#endif
...
FTS_BOOL i2c_read_interface(FTS_BYTE bt_ctpm_addr, FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
    ...
    if(ret<=0)
    {
        TS_DBG("[TSP]i2c_read_interface error\n");
        return FTS_FALSE;
    }
...
FTS_BOOL i2c_write_interface(FTS_BYTE bt_ctpm_addr, FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
    ...
    if(ret<=0)
    {
        TS_DBG("[TSP]i2c_write_interface error line = %d, ret = %d\n", __LINE__, ret);
        return FTS_FALSE;
    }
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
    ...
    {
        TS_DBG("[TSP] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0],reg_val[1]);
    }
    else
    {
        TS_DBG("%s: ERR_READID, ID1 = 0x%x,ID2 = 0x%x\n", __func__,reg_val[0],reg_val[1]);
        return ERR_READID;
        //i_is_new_protocol = 1;
    }
    ...
    for (j=0;j<packet_number;j++)
    {
        ...
        {
              TS_DBG("[TSP] upgrade the 0x%x th byte.\n", ((unsigned int)j) * FTS_PACKET_LENGTH);
        }
    }
...
#endif
    if (ret < 0) {
		TS_DBG("%s read_data i2c_rxdata failed: %d\n", __func__, ret);
		return ret;
	}
...
PROXIMITY_HANDLE:
    if (1 == !!PROXIMITY_SWITCH)
    {
        ...
        {
            PROXIMITY_STATE = !!buf[1]; // 1;
        }
        else if (FT5X0X_PROX_FAR == buf[1])
        {
            PROXIMITY_STATE = !buf[1]; // 0;
        }

        spin_unlock(&proximity_state_lock);

        TS_DBG(">>>> reg(0xB0) = 0x%.2X, buf[1] is 0x%.2X, PROXIMITY_STATE = %d <<<<\n", reg_val, buf[1], PROXIMITY_STATE);
...
static unsigned char suspend_flags = 0;//add by kevin 2014-02-27
...
static void ft5x0x_ts_suspend(struct early_suspend *handler)
{
	...
	{
		TS_DBG("FT6206 proximity already open,suspend do nothing!\n");
		suspend_flags = 0;
	}
	else
	{
...
//ktang add for tp proximity
	...
	input_dev->name		= "focaltech_ts";		//dev_name(&client->dev)
