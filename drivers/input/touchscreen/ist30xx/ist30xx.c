...
static irqreturn_t ist30xx_irq_thread(int irq, void *ptr)
{
	...
	if (ret)
	{
		printk("[TSP]Not able to get pisition \n");
		goto irq_err;
	}
...
#endif
	{
		if ((finger_cnt > data->max_fingers) || (key_cnt > data->max_keys)) {
				   ...
				   const struct i2c_device_id * id)
{
	...
	printk("[TSP] IST3038 Probe Called\n");
	...
	while (data->chip_id != IST30XXB_CHIP_ID) {
		...
		//printk("[TSP]Chip id of the chip is : %d\n", data->chip_id);
	...
	if (retry == 0)
	{
		...
		tsp_err("%s(), IST3038 IC not recognised \n", __func__);
	}
...
#ifdef CONFIG_OF
static const struct of_device_id ist30xx_ts_of_match[] = {
...
#endif
#ifdef CONFIG_OF
                .of_match_table = ist30xx_ts_of_match,
#endif
	},
};
