...
LOCAL uint32_t set_gc0309_anti_flicker(uint32_t param )
{
    switch (param)
    {
        case FLICKER_50HZ:
            ...
            GC0309_WriteReg(0xe3, 0x60);   //anti-flicker step [7:0]

            GC0309_WriteReg(0xe4, 0x02);   //exp level 1  16.67fps
            GC0309_WriteReg(0xe5, 0xa0); 
            GC0309_WriteReg(0xe6, 0x03);   //exp level 2  12.5fps
            GC0309_WriteReg(0xe7, 0x20); 
            GC0309_WriteReg(0xe8, 0x04);   //exp level 3  8.33fps
            GC0309_WriteReg(0xe9, 0x80); 
...
SENSOR_REG_T GC0309_video_mode_nor_tab[][15]=
{
    ...
    {
	{0x01,0x2c},{0x02,0x98},{0x0f,0x02},{0xe2,0x00},{0xe3,0x60},{0xe4,0x02},{0xe5,0xa0},
	{0xe6,0x03},{0xe7,0x20},{0xe8,0x06},{0xe9,0x40},{0xea,0x09}, {0xeb,0xc4}, {0xec,0x20},{0xff,0xff} 
	},    
    //vodeo mode     
    {
