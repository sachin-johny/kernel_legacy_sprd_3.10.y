...
LOCAL const SENSOR_REG_T ov5640_640X480_new[] = {
	...
	{0x3036, 0x40},		/* PCLK manual divider*/
...
LOCAL const SENSOR_REG_T ov5640_video_mode_tab[][8]=
{
	...
	{
		{0x3036, 0x40},/* 0x50: 35fps, 0x46: 30 fps*/
		{0x3c01, 0x80},
		{0x3c00, 0x04},
		{0x3a08, 0x01},
		{0x3a09, 0x04},
...
#define g_Preview_FrameRate 300
	...
	if (g_flash_mode_en) {
		printk("_ov5640_BeforeSnapshot: g_flash_mode_en: %d, g_auto_flash_mode_en: %d\n", 
			g_flash_mode_en, g_auto_flash_mode_en);
...
LOCAL uint32_t _ov5640_flash(uint32_t param)
{
	...
	printk("SENSOR: _ov5640_flash:param=0x%x.\n", param);
	/* enable flash, disable in _ov5640_BeforeSnapshot */
	g_flash_mode_en = param;
	if(param == FLASH_AUTO) {
		value = Sensor_ReadReg(0x56a1);
		if(value < 15) {
			printk("_ov5640_flash: open flash, r(0x56a1) = %d\n", value);
			g_auto_flash_mode_en = 1;
			Sensor_SetFlash(FLASH_OPEN);
		}else {
			printk("_ov5640_flash: close flash, r(0x56a1) = %d\n", value);
...
LOCAL uint32_t _ov5640_AutoFocusZone(SENSOR_EXT_FUN_PARAM_T_PTR param_ptr)
{
	...
	if (SENSOR_SUCCESS == rtn) {
		...
		if(SENSOR_SUCCESS==rtn)
		{
			...
			if(0x00==reg_value)
			{
				printk("SENSOR: _ov5640_AutoFocusZone fail! \n");
				rtn=SENSOR_FAIL;
			}
		}
...
LOCAL uint32_t _ov5640_AutoFocusMultiZone(SENSOR_EXT_FUN_PARAM_T_PTR param_ptr)
{
	...
	for (i = 0; i < zone_cnt; i++) {
		...
		printk("SENSOR: _ov5640_AutoFocusMultiZone 0:x=%d,y=%d.\n",
		...
		printk("SENSOR: _ov5640_AutoFocusMultiZone:x=%d,y=%d.\n",
		       ext_param[i].zone.x, ext_param[i].zone.y);
	}
	...
	if(SENSOR_SUCCESS==rtn)
	{
		...
		if(0x00==reg_value)
		{
			printk("SENSOR: _ov5640_AutoFocusMultiZone fail! \n");
			rtn=SENSOR_FAIL;
		}
	}
	return rtn;
...
LOCAL uint32_t _ov5640_AutoFocusMacro(SENSOR_EXT_FUN_PARAM_T_PTR param_ptr)
{
	...
	if(SENSOR_SUCCESS==rtn)
	{
		...
		if(0x00==reg_value)
		{
			printk("SENSOR: _ov5640_AutoFocusMacro fail! \n");
			rtn=SENSOR_FAIL;
		}
	}
...
LOCAL uint32_t _ov5640_StartAutoFocus(uint32_t param)
{
	...
	printk("SENSOR: _ov5640_StartAutoFocus param =%d \n", ext_ptr->param);
...
LOCAL int _ov5640_init_firmware(uint32_t param)
{
	...
	printk("SENSOR: _ov5640_init_firmware: E!.\n");

	//printk("SENSOR: 0x3029=0x%x,0x3000=0x%x,0x3004=0x%x.\n",Sensor_ReadReg(0x3029),reg_val_1,reg_val_2);
	//printk("SENSOR: 0x8000=0x%x,0x8002=0x%x,0x8f57=0x%x.\n",Sensor_ReadReg(0x8000),Sensor_ReadReg(0x8002),Sensor_ReadReg(0x8f57));
	return ret;
}
...
LOCAL uint32_t _ov5640_ExtFunc(uint32_t ctl_param)
{
	...
	printk("SENSOR: _ov5640_ExtFunc cmd:0x%x  \n", ext_ptr->cmd);
