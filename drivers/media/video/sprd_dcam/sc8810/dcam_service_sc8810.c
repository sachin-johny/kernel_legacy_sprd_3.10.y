/*
* drivers/media/video/sprd_dcam/dcam_service_sc8810.c
...
LOCAL struct clk *g_dcam_clk = NULL; //for power manager
...
static int dcam_set_mclk(ISP_CLK_SEL_E clk_sel)
{
	...
	printk("DCAM:dcam_set_mclk,dcam usecount=%d .\n", g_dcam_clk->parent->usecount);
	...
	{//need to wait the parent
		...
		if(ret)
		{
			DCAM_TRACE_ERR("DCAM:dcam_set_mclk,clock[%s]: clk_set_parent() failed!parent: %s, usecount: %d.\n", g_dcam_clk->name, clk_parent->name, g_dcam_clk->usecount);
			return -EINVAL;
		}		
	}
	...
	if(ret)
	{
		DCAM_TRACE_ERR("DCAM:dcam_set_mclk,clock[%s]: clk_enable() failed!\n", g_dcam_clk->name);
	}
	else
	{
...
int32_t  _ISP_ServiceStartPreview(void)
{
	...
	if(ISP_ROTATION_0 != s->display_rotation)
	{
		...
		{
		 	if(((ISP_ROTATION_90 == s->display_rotation)||(ISP_ROTATION_270 == s->display_rotation))&&(0==(disp_size.h&0x7)))
		 	{
	...
	printk("_ISP_ServiceStartPreview:hsync=%d,vsync=%d",cap_sync.hsync_pol,cap_sync.vsync_pol);
	rtn_drv = ISP_DriverCapConfig(s->module_addr, 
	                                                           ISP_CAP_SYNC_POL, 
	                                                           (void*)&cap_sync);
	ISP_RTN_IF_ERR(rtn_drv);
	...
	{
		isp_get_path2();
		printk("DCAM:_ISP_ServiceStartPreview,use path2 when preview.\n");
		/*Set Path2*/
...
exit:
	...
	if(rtn_drv)
	{
		DCAM_TRACE_ERR("DCAM: start preview: driver error code 0x%x",rtn_drv);	
	}
	else
	{
...
static uint32_t _ISP_ServiceGetXYDeciFactor(uint32_t *src_width, uint32_t *src_height, uint32_t dst_width, uint32_t dst_height)
{
	...
	DCAM_TRACE("DCAM:_ISP_ServiceGetXYDeciFactor.src_w=%d,src_h=%d,dst_w=%d,dst_h=%d .\n",
...
static int _ISP_GetSensorInterfaceInfo(void)
{
	...
	sensor_interface = sensor_info_ptr->sensor_interface;
		...
		default:
		{
			DCAM_TRACE_ERR("DCAM:_ISP_GetSensorInterfaceInfo sensor interface is default!");
			break;
		}
	}
...
static ISP_DCAM_PATH1_OUT_FORMAT_E dcam_outformat_convert(DCAM_DATA_FORMAT_E input_format)
{
	...
	switch(input_format)
	{
		...
		case DCAM_DATA_JPEG:
			...
			break;
		default:
			DCAM_TRACE_ERR("DCAM:dcam_outformat_convert param error,input_format=%d .\n",input_format);
			break;
	}
...
static int ISP_ServiceSetParameters(void)
{
	...
	dst_img_size.h = g_dcam_param.display_rect.h;
         ...
         if(dst_img_size.w*g_dcam_param.input_size.h<g_dcam_param.input_size.w*dst_img_size.h)//full screen
         	{
		trim_width = dst_img_size.w*g_dcam_param.input_size.h/dst_img_size.h;
		trim_height = g_dcam_param.input_size.h;
         	}
	else
	{
		trim_width = g_dcam_param.input_size.w;
		trim_height = g_dcam_param.input_size.w*dst_img_size.h/dst_img_size.w;
	}     
	...
	s->cap_input_range.x = ( g_dcam_param.input_size.w-trim_width)/2 ;//g_dcam_param.input_rect.x;
	s->cap_input_range.x &= ~1;
	s->cap_input_range.y = (g_dcam_param.input_size.h-trim_height)/2;//g_dcam_param.input_rect.y;
	s->cap_input_range.y &= ~1;

	trim_width = DCAMERA_WIDTH(trim_width);
	trim_height = DCAMERA_HEIGHT(trim_height);
	s->cap_input_range.w = trim_width;//g_dcam_param.input_rect.w;
	s->cap_input_range.h = trim_height;//g_dcam_param.input_rect.h;
	s->cap_output_size.w = trim_width;//s->cap_input_range.w;
	s->cap_output_size.h = trim_height;//s->cap_input_range.h;
...
void ISP_ServiceStopPreview(void)
{
	...
	{		
		DCAM_TRACE_ERR("DCAM:ISP_ServiceStopPreview: state is not preview.\n");
		return ;
	}
	...
	{
		printk("DCAM:_ISP_ServiceClose, ISP_MODE_PREVIEW_EX put path2 .\n");
		isp_put_path2();
	}
...
void dcam_powerdown(uint32_t sensor_id,uint32_t value)
{
	...
	printk("DCAM Service:dcam_powerdown:module addr:0x%x,value:%d\n",s->module_addr,value);
