/*
* drivers/media/video/sprd_isp/dcam_drv_sc8810.c
 * Dcam driver based on sc8810
 *
...
{
	ISP_REG_T                 *p_isp_reg = (ISP_REG_T*)base_addr;
	ISP_ISR_FUNC_PTR          user_func = s_isp_mod.user_func[ISP_IRQ_NOTICE_PATH2_DONE];
	...
	printk("DCAM DRV: _ISP_ISRPath2Done y,u,v = {0x%x,0x%x,0x%x} ,width,height = {%d, %d}",
	              frame_curr->yaddr,
	              frame_curr->uaddr,
	              frame_curr->vaddr,
	              frame_curr->width,
...
static  uint32_t _ISP_DriverGetSubSampleFactor(
																uint32_t* src_width, 
																uint32_t* src_height,
																uint32_t dst_width,
																uint32_t dst_height
															)
{
	if(*src_width > (dst_width * ISP_PATH_SC_COEFF_MAX) || *src_height > (dst_height * ISP_PATH_SC_COEFF_MAX) )
	{
		...
		*src_height = *src_height >> 1;
		return _ISP_DriverGetSubSampleFactor(src_width,src_height,dst_width,dst_height) + ISP_PATH_SUB_SAMPLE_FACTOR_BASE;
	}
	else
	{
		return 0;
...
static int32_t  _ISP_DriverCalcSC2Size(void)
{
	...
	if(p_path->input_rect.w * ISP_PATH_SC_COEFF_MAX < p_path->output_size.w ||p_path->input_rect.h * ISP_PATH_SC_COEFF_MAX < p_path->output_size.h) 
	{
		rtn = ISP_DRV_RTN_PATH_SC_COEFF_ERR;
	}
	else if(p_path->input_rect.w > p_path->output_size.w * ISP_PATH_SC_COEFF_MAX ||p_path->input_rect.h > p_path->output_size.h * ISP_PATH_SC_COEFF_MAX)
	{
		p_path->sc_input_size.w = p_path->input_rect.w;
		p_path->sc_input_size.h = p_path->input_rect.h;		

		p_path->sub_sample_factor = _ISP_DriverGetSubSampleFactor(&p_path->sc_input_size.w,
		...
		{
			p_path->sc_input_size.w = (uint32_t)(p_path->input_rect.w / (uint32_t)( 1 << p_path->sub_sample_factor));
			p_path->sc_input_size.h = (uint32_t)(p_path->input_rect.h / (uint32_t)( 1 << p_path->sub_sample_factor));			
...
{  
	...
	if(SCI_NULL == pTmpBuf)
	{		
		DCAM_DRV_ERR("DCAM DRV ERROR:_ISP_DriverGenScxCoeff,kmalloc addr is NULL!");
	...
	DCAM_DRV_TRACE("DCAM DRV:_ISP_DriverGenScxCoeff i_w/i_h/o_w/o_h = {%d, %d, %d, %d,}",
		...
		ISP_DRV_SCALE_COEFF_TMP_SIZE)))
	{
		kfree(pTmpBuf);
		pTmpBuf = SCI_NULL;
		DCAM_DRV_ERR("DCAM DRV ERROR: _ISP_DriverGenScxCoeff GenScaleCoeff error!");    
	...
	{    
		...
		{               
		 ...
		 DCAM_DRV_TRACE("DCAM DRV: _ISP_DriverGenScxCoeff path1 Domain = 0x%x", 
	...
	else 
	{
		do
		{
	    	...
	    	DCAM_DRV_TRACE("DCAM DRV: _ISP_DriverGenScxCoeff path 2 Domain = 0x%x", 
...
//		DCAM_DRV_TRACE("DCAM DRV: Coeff V[%d] = 0x%x.\n", i, *pVCoeff);
	...
	DCAM_DRV_TRACE("DCAM DRV: _ISP_DriverGenScxCoeff V[%d] = 0x%x", i,  (*pVCoeff) & 0x0F);
	DCAM_DRV_TRACE("DCAM DRV:_ISP_DriverGenScxCoeff V[%d] = 0x%x", i, p_isp_reg->dcam_path_cfg_u.mBits.ver_down_tap);
    
	if(ISP_PATH1 == idxScx)        
	{
		 p_isp_reg->dcam_cfg_u.mBits.path1_clock_switch = ISP_CLK_DOMAIN_DCAM;
		DCAM_DRV_TRACE("DCAM DRV: _ISP_DriverGenScxCoeff Domain = 0x%x", p_isp_reg->dcam_cfg_u.mBits.path1_clock_switch);
	}
	else
	{
...
int32_t ISP_DriverModuleDisable(uint32_t ahb_ctrl_addr) // must be AHB general control register base address
{
	ISP_DRV_RTN_E     rtn = ISP_DRV_RTN_SUCCESS;    
...
int32_t ISP_DriverStart(uint32_t base_addr)
{
	...
	else
	{
		...
		ISP_DRV_RTN_IF_ERR;	
                  ...
                  _ISP_DriverIrqClear(base_addr,ISP_IRQ_LINE_MASK);
		_ISP_DriverIrqEnable(base_addr, ISP_IRQ_LINE_MASK);
		//set little endian for path2
		p_isp_reg->endian_sel_u.mBits.review_output_endian_y    = ISP_MASTER_ENDIANNESS_LITTLE;
		p_isp_reg->endian_sel_u.mBits.review_output_endian_uv = ISP_MASTER_ENDIANNESS_LITTLE;
	}
#ifdef DCAM_DRV_DEBUG
	_ISP_GetReg();	
#endif
...
int32_t ISP_DriverStop(uint32_t base_addr)
{
	...
	switch(s_isp_mod.isp_mode)
	{
		...
		case ISP_MODE_PREVIEW:
		{
			...
			printk("###ISP DRV:ISP_DriverStop wait the last interrupt.value: 0x%x, count: %d\n", value, count);
			if(value & 0x10)
			{
			DCAM_DRV_TRACE("DCAM DRV:ISP_DriverStop wait the last interrupt.\n");					
			break;
			}				
			msleep(20);
			}			
...
int32_t ISP_DriverCapConfig(uint32_t base_addr, ISP_CFG_ID_E isp_cfg_id, void* param)
{
	ISP_DRV_RTN_E   rtn = ISP_DRV_RTN_SUCCESS;
	ISP_REG_T   	     *p_isp_reg = (ISP_REG_T*)base_addr;
	uint32_t                   value;
	...
	switch(isp_cfg_id)
	{
		case ISP_CAP_CCIR565_ENABLE:
		{
			...
			p_isp_reg->cap_ctrl_u.mBits.ccir_656  = *(uint32_t*)param ? 1 : 0;
			 break;
	    	}
		...
		{
			ISP_CAP_PATTERN_E cap_yuu_pat = *(ISP_CAP_PATTERN_E*)param;
			ISP_CHECK_PARAM_ZERO_POINTER(param); 
			  
			if(cap_yuu_pat < ISP_PATTERN_MAX)
			{                
				p_isp_reg->cap_ctrl_u.mBits.yuv_type = cap_yuu_pat;
			}
			else
			{
		...
		case ISP_CAP_PRE_SKIP_CNT:
		{
			...
			{
				p_isp_reg->cap_frm_ctrl_u.mBits.pre_skip_cnt = skip_num;
			}
			else
			{
		...
		case ISP_CAP_FRM_DECI:
		{
			...
			{
				p_isp_reg->cap_frm_ctrl_u.mBits.cap_frm_deci = deci_factor;
			}
			else
			{
		...
		case ISP_CAP_INPUT_RECT:
		{
			...
			{                
				p_isp_reg->cap_start_u.mBits.start_x = p_rect->x << 1;
				p_isp_reg->cap_start_u.mBits.start_y = p_rect->y;
				p_isp_reg->cap_end_u.mBits.end_x = ((p_rect->w + p_rect->x )<< 1) - 1;
				p_isp_reg->cap_end_u.mBits.end_y = p_rect->y + p_rect->h - 1;
			}                	
			break;
		}
		case ISP_CAP_IMAGE_XY_DECI:
		{
			...
			{	
				p_isp_reg->cap_img_deci_u.mBits.cap_deci_x = p_cap_dec->x_factor;
				p_isp_reg->cap_img_deci_u.mBits.cap_deci_y = p_cap_dec->y_factor;
			}
			break;
		}
		...
		{              
			...
			{
				p_isp_reg->cap_ctrl_u.mBits.sensor_mode = cap_input_format;
			}
		...
		{
			ISP_CAP_IF_MODE_E cap_if_mode = *(ISP_CAP_IF_MODE_E*)param;
			...
			{
				p_isp_reg->cap_ctrl_u.mBits.cap_if_mode = cap_if_mode;
			}
			break;
		}
		case ISP_CAP_SPI_ORIG_WIDTH:
		{
			...
			{			
			p_isp_reg->cap_spi_cfg_u.mBits.spi_orig_width = width;
			}
			break;
		}
		case ISP_CAP_SENSOR_MODE:
		{
			ISP_CAP_SENSOR_MODE_E sensor_mode = *(ISP_CAP_SENSOR_MODE_E*)param;
			...
			{
				p_isp_reg->cap_ctrl_u.mBits.sensor_mode = sensor_mode;
			}				
			break;
		}
		default:
...
int32_t ISP_DriverCapGetInfo(uint32_t base_addr, ISP_CFG_ID_E id, void* param)
{
	...
	switch(id)
	{
		case ISP_CAP_FRM_COUNT_GET:
			*(uint32_t*)param =p_isp_reg->cap_frm_ctrl_u.mBits.cap_frm_cnt;
			break;
		case ISP_CAP_JPEG_GET_NUM:
			*(uint32_t*)param = ISP_DRV_RTN_IO_ID_UNSUPPORTED;
			break;    
		case ISP_CAP_JPEG_GET_LENGTH:
			*(uint32_t*)param = p_isp_reg->cap_frm_size_u.mBits.cap_frm_size;
			break;
		default:
			rtn = ISP_DRV_RTN_IO_ID_UNSUPPORTED;
			break;            
...
int32_t ISP_DriverPath1Config(uint32_t base_addr, ISP_CFG_ID_E id, void* param)
{
	...
	switch(id)
	{
		...
		case ISP_PATH_INPUT_SIZE:
		{
			...
			DCAM_DRV_TRACE("DCAM DRV: ISP_PATH_INPUT_SIZE w: %d, h: %d.\n", p_size->w, p_size->h);  
			...
			{
				p_isp_reg->dcam_src_size_u.mBits.src_size_x = p_size->w;
				p_isp_reg->dcam_src_size_u.mBits.src_size_y = p_size->h;                
		...
		case ISP_PATH_INPUT_RECT:
		{
			...
			{
				p_isp_reg->dcam_trim_start_u.mBits.start_x = p_rect->x;
				p_isp_reg->dcam_trim_start_u.mBits.start_y = p_rect->y;
				p_isp_reg->dcam_trim_size_u.mBits.size_x   = p_rect->w;
				p_isp_reg->dcam_trim_size_u.mBits.size_y   = p_rect->h;
		...
		case ISP_PATH_OUTPUT_SIZE:
		{
			...
			{
				p_isp_reg->dcam_des_size_u.mBits.des_size_x = p_size->w;
				p_isp_reg->dcam_des_size_u.mBits.des_size_y = p_size->h;		
				p_path->output_size.w = p_size->w;
				p_path->output_size.h = p_size->h;				
			}
			DCAM_DRV_TRACE("DCAM DRV: output size w: %d, h: %d, reg DCAM_DES_SIZE: 0x %x.\n", p_size->w, p_size->h, _pard(DCAM_DES_SIZE));
			break;
		}  
		case ISP_PATH_OUTPUT_FORMAT:
		{
			...
			else
			{
				...
				{
					p_isp_reg->dcam_path_cfg_u.mBits.cam1_odata_format = 0;
				}
				else if(ISP_DATA_YUV420 == format)
				{
					p_isp_reg->dcam_path_cfg_u.mBits.cam1_odata_format = 1;
				}
				else
				{
					p_isp_reg->dcam_path_cfg_u.mBits.cam1_odata_format = 2;
				}
		...
		case ISP_PATH_SUB_SAMPLE_EN:
		{
			...
			p_isp_reg->dcam_path_cfg_u.mBits.cam1_deci_eb = p_path->sub_sample_en;
			DCAM_DRV_TRACE("DCAM DRV: path1 1/2 sub_sample_en..\n ");        	  	
			break;
		}      	        
		case ISP_PATH_SUB_SAMPLE_MOD:
		{            
			rtn = ISP_DRV_RTN_IO_ID_UNSUPPORTED;	
			DCAM_DRV_TRACE("DCAM DRV:don't support ISP_PATH_SUB_SAMPLE_MOD of path1.\n");
			break;           
		}	
		case ISP_PATH_UV420_AVG_EN:
		{
			...
			p_isp_reg->dcam_path_cfg_u.mBits.cam1_uv420_avg_eb = avg_eb;
			break;
		}
		case ISP_PATH_DITHER_EN:
		{
			...
			p_isp_reg->dcam_path_cfg_u.mBits.cam1_dither_eb = dither_eb;
		...
		case ISP_PATH_DATA_ENDIAN:
		{
			...
			p_isp_reg->endian_sel_u.mBits.dcam_output_endian_y = endian_param.endian_y;
			p_isp_reg->endian_sel_u.mBits.dcam_output_endian_uv = endian_param.endian_uv;
			break;
		}
		default:
			break;
...
int32_t ISP_DriverPath2Config(uint32_t base_addr, ISP_CFG_ID_E id, void* param)
{
	...
	switch(id)
	{
		...
		case ISP_PATH_INPUT_RECT:
		{
			...
			if(p_rect->x > ISP_PATH_FRAME_WIDTH_MAX ||  p_rect->y > ISP_PATH_FRAME_HEIGHT_MAX ||
			    p_rect->w > ISP_PATH_FRAME_WIDTH_MAX || p_rect->h > ISP_PATH_FRAME_HEIGHT_MAX )
			{
				rtn = ISP_DRV_RTN_PATH_TRIM_SIZE_ERR;    
			}
			else
			{
				...
				ISP_MEMCPY((void*)&p_path->input_rect,(void*)p_rect,sizeof(ISP_RECT_T));
			}
			break;
		}
		case ISP_PATH_INPUT_ADDR:
		...
		case ISP_PATH_OUTPUT_ADDR:
		{
			...
			{     
				if(p_path->output_frame_count > ISP_PATH2_FRAME_COUNT_MAX - 1)     	
				{
					rtn = ISP_DRV_RTN_PATH_FRAME_TOO_MANY;  
				}
				else
		...
		case ISP_PATH_SUB_SAMPLE_MOD:
		{
			...
			{
				p_isp_reg->rev_path_cfg_u.mBits.sub_sample_eb = sub_sameple_mode;
		...
		case ISP_PATH_SWAP_BUFF:
		{
			...
			{	
				...
				p_isp_reg->frm_addr_2_u.mBits.frm_addr_2 = p_addr->yaddr;
				p_isp_reg->frm_addr_3_u.mBits.frm_addr_3 = p_addr->uaddr;
			}
			break;
		}		
		case ISP_PATH_LINE_BUFF:
		{
			...
			{	
				...
				p_isp_reg->frm_addr_6_u.mBits.frm_addr_6 = p_addr->yaddr;
			}
			break;
		}			    
...
int32_t ISP_DriverPath2GetInfo(uint32_t base_addr, ISP_CFG_ID_E id, void* param)
{
	...
	switch(id)       
	{
		...
		case ISP_PATH_SLICE_OUT_HEIGHT:
			ISP_CHECK_PARAM_ZERO_POINTER(param);
			*(uint32_t*)param = (uint32_t)p_isp_reg->slice_ver_cnt_u.mBits.slice_line_output;
			break;			
		default:
			rtn = ISP_DRV_RTN_IO_ID_UNSUPPORTED;
			break;
...
int32_t ISP_DriverSetMode(uint32_t base_addr, ISP_MODE_E mode)
{
	...
	switch(mode)
	{
		case ISP_MODE_PREVIEW:            
			...
			p_isp_reg->dcam_path_cfg_u.mBits.cap_mode = DCAM_CAP_MODE_MULTIPLE;
			 break;
		case ISP_MODE_PREVIEW_EX:			
		...
		case ISP_MODE_CAPTURE:            
			...
			p_isp_reg->dcam_cfg_u.mBits.cam_path2_eb = 0;
			p_isp_reg->dcam_cfg_u.mBits.review_path_eb = 0;
			p_isp_reg->dcam_path_cfg_u.mBits.cap_mode = DCAM_CAP_MODE_SINGLE;
			break;   
		default:
			rtn = ISP_DRV_RTN_MODE_ERR;
			break;
...
//	DCAM_TRACE("DCAM DRV:ISP_DriverSetBufferAddress:base_addr=0x%x,path_id = %d.\n",base_addr,s_isp_mod.isp_mode);
	...
	{
		DCAM_TRACE_ERR("DCAM DRV:ISP_DriverSetBufferAddress ERROR:isp mode = %d .\n",s_isp_mod.isp_mode);
	}
