...
PUBLIC SENSOR_INFO_T g_GT2005_yuv_info =
{
	...
	SENSOR_I2C_REG_16BIT,//BIT_2,							// bit0: 0: i2c register value is 8 bit, 1: i2c register value is 16 bit
 ...
 **---------------------------------------------------------------------------*/
/*LOCAL void GT2005_WriteReg( uint16_t  subaddr, uint8_t value )
{
	uint8_t i2c_cmd[3];
	i2c_cmd[0] = subaddr >> 8;
	i2c_cmd[1] = subaddr & 0xff;
	i2c_cmd[2] = value;
	
	I2C_WriteCmdArr(GT2005_I2C_ADDR_W, i2c_cmd, 3, SCI_TRUE);
}
LOCAL uint8_t GT2005_ReadReg( uint16_t  subaddr)
{
	uint8_t value = 0;
	...
	I2C_WriteCmdArr(GT2005_I2C_ADDR_W, i2c_cmd, 2, SCI_TRUE);
	I2C_ReadCmd(GT2005_I2C_ADDR_R, &value, SCI_TRUE);
...
LOCAL uint32_t GT2005_Power_On(uint32_t power_on)
{
    ...
    uint32_t 				reset_width=g_GT2005_yuv_info.reset_pulse_width;	    
    ...
    SENSOR_TRACE("set_GT2005_Power_On");
...
LOCAL uint32_t Set_GT2005_Preview_Mode(uint32_t preview_mode)
{
	switch (preview_mode)
	{
		...
		{
			Sensor_WriteReg(0x0312, 0x08);	// 1/2 Frame rate	
			break;
		}
...
{ 
	...
	Sensor_WriteReg(0x0118 , 0x40);
	Sensor_WriteReg(0x0119 , 0x01);
	Sensor_WriteReg(0x011a , 0x04);	
	Sensor_WriteReg(0x011B , 0x00);

	Sensor_WriteReg(0x0313 , 0x35); 
	Sensor_WriteReg(0x0314 , 0x36);
...
{	
       ...
       SENSOR_TRACE("SENSOR: set_ae_awb_enable: ae_enable = %d", ae_enable);
       SENSOR_TRACE("SENSOR: set_ae_awb_enable: ae_enable = %d", awb_enable);
	return 0;
}*/
