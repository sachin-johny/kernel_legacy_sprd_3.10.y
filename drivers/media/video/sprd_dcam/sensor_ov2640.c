 ...
 ** 						   Macro Define
 **---------------------------------------------------------------------------*/
#define OV2640_I2C_ADDR_W		0x30//0x60
#define OV2640_I2C_ADDR_R		0x30//0x61
...
//    SCI_ASSERT(PNULL != sensor_reg_ptr);
    ...
    {
        Sensor_WriteReg_8bits(sensor_reg_ptr[i].reg_addr, (uint8_t)sensor_reg_ptr[i].reg_value);
    }
...

{
	...
	printk("[OV2640_Identify:pid_value=0x%x]\n",pid_value);
	...
	if(OV2640_PID_VALUE == pid_value)
	{
     ...
     //   ver_value = Sensor_ReadReg(OV2640_VER_ADDR);
		...
		printk("[OV2640_Identify:ver_value=0x%x]\n",ver_value);
		...
		{
			SENSOR_TRACE("OV2640_Identify: That is OV%x%x sensor !", pid_value, ver_value);
		}
		ret_value = 0;
	}
...
/*  uint16	PrvMaxExp = 1248;*/
    ...
    uint32_t  preview_xclk = s_OV2640_resolution_Tab_YUV[sensor_preview_mode].xclk_to_sensor;
    uint32_t  snapshot_xclk = s_OV2640_resolution_Tab_YUV[sensor_snapshot_mode].xclk_to_sensor;
...
//    SCI_ASSERT(preview_xclk && snapshot_xclk);
 ...
 // 4 . Calculate exposure/gain
	...
	PrvExp += tmp<<10;
       ...
       TgtExp = (PrvExp * snapshot_xclk / preview_xclk ) * (2* 4 * 1190) / (3 * 1922);
...
LOCAL uint32_t OV2640_chang_image_format(uint32_t param)
{
    ...
    default:
        SENSOR_TRACE(0, ("OV2640 only support SENSOR_IMAGE_FORMAT_JPEG & SENSOR_IMAGE_FORMAT_YUV422, input is %d", param));
        break;
    }
...
LOCAL uint32_t OV2640_check_image_format_support(uint32_t param) 
{
    ...
    default:
        SENSOR_TRACE("OV2640 only support SENSOR_IMAGE_FORMAT_JPEG & SENSOR_IMAGE_FORMAT_YUV422, input is %d", param);
        break;
    }
...
LOCAL uint32_t OV2640_pick_out_jpeg_stream(uint32_t param)
{
    ...
    SENSOR_TRACE("ov3640: jpeg capture head: 0x%x, 0x%x", *((uint8*)p_frame), *((uint8*)p_frame + 1));
    ...
    if(i < buf_len){
        SENSOR_TRACE("ov3640: Found the jpeg tail at %d: 0x%x 0x%x", i+1, *((uint8*)p_frame + i), *((uint8*)p_frame + i + 1));
    }
    else{
...
#if 0
LOCAL uint32_t OV2640_zoom(uint32_t param)
{
	uint16_t i;
	uint32_t zoom_level,sensor_mode;
	uint32_t* pint32 = (uint32_t *)param;
...
//	SCI_ASSERT(zoom_level <= 4 && zoom_level>=1);
//	SCI_ASSERT(sensor_mode <= 3 && sensor_mode>=2);
//	SCI_ASSERT(PNULL != sensor_reg_ptr);
	SENSOR_TRACE("OV2640_zoom: mode=%d,level = %d", sensor_mode,zoom_level);
