...
const SENSOR_REG_T HI253_YUV_640X480[]= {
	...
	//Initial Start
	{0x03, 0x00},
	{0x10, 0x19}, //0401//0x11}, //jinkay//{0x00}, //Sub 1/2_Preview1 Mode, VSync Type2  00
	...
	{0x12, 0x30},
	{0x13, 0x0a}, //bright2 on, contrast on
	{0x20, 0x00},
	...
	{0x83, 0x04}, //exp 16//0x01}, //EXP Normal 33.33 fps 
	{0x84, 0x1d}, //0x5f}, 
	{0x85, 0xf0}, //0x00}, 
	...
	{0x88, 0x03}, //0x02}, //jinkay////0x04}, //EXP Max 10.00 fps 
	{0x89, 0x33}, //0xbe}, //jinkay//0x92}, 
	{0x8a, 0x00}, //0x00}, 
	...
	// PAGE 0
	{0x03, 0x00},
	{0x0e, 0x03}, // 4x//0x03}, //PLL On
	{0x0e, 0x73},      //0x73}, //PLLx2
	...
	{0x20, 0x05}, //jinkay//0x02},
	{0x21, 0x00}, //jinkay//0x84}, //80},
	{0x22, 0x01}, //jinkay//0x01},
	{0x23, 0xe0}, //jinkay//0xe0},
	{0x24, 0x00}, //jinkay//0x00},
	{0x25, 0x00}, //jinkay//0x04}, //00},
	{0x26, 0x00}, //jinkay//0x00},
	{0x27, 0x00}, //jinkay//0x00},
	{0x28, 0x05}, //jinkay//0x02},
	{0x29, 0x00}, //jinkay//0x84}, //80},
	{0x2a, 0x01}, //jinkay//0x01},
	{0x2b, 0xe0}, //jinkay//0xe0},
	{0x2c, 0x0a}, //jinkay//0x14},
	{0x2d, 0x00}, //jinkay//0x00},
	{0x2e, 0x0a}, //jinkay//0x14},
	{0x2f, 0x00}, //jinkay//0x00},
	{0x30, 0x44}, //jinkay//0x66},
...
{  
	...
	{0x03,0x00},//Page 0
	{0x01,0xf8}, //0},//0x70},//Sensor sleep off, Output Drivability Max
...
{
	{0x03,0x00},                                          
	{0x01,0x79}, //0x71},//sleep On                                
	{0x0e,0x00},//20091015 pll off  
	...
	{0x03,0x00},                                          
	{0x10,0x08}, //0401//0x00},//100208 Vsync type2
	{0x11,0x93},
	{0x12,0x04},//                            
...
{	
        // COMMON INIT
        {ADDR_AND_LEN_OF_ARRAY(HI253_YUV_640X480),0,0,24,SENSOR_IMAGE_FORMAT_YUV422},
        // YUV422 PREVIEW 1
...
LOCAL SENSOR_IOCTL_FUNC_TAB_T s_hi253_ioctl_func_tab = 
{
        ...
        set_hi253_video_mode, // set video mode
...
LOCAL uint32_t HI253_Identify(uint32_t param)
{
        ...
        SENSOR_PRINT("HI253_Identify: it is HI253");
...
LOCAL uint32_t set_hi253_ae_awb_enable(uint32_t ae_enable, uint32_t awb_enable)
{
	...
	SENSOR_PRINT("set_ae_awb_enable: ae=%d awb=%d", ae_enable, awb_enable);
	return 0;
}
#endif
#if 0
...
LOCAL uint32_t HI253_set_brightness(uint32_t level)
{
        ...
        if(level >= 7)
        {
                SENSOR_PRINT("HI253_set_brightness:param error,level=%d .\n",level);
        }

        Sensor_WriteReg(0x03,0x10);
        Sensor_WriteReg(0x12,(Sensor_ReadReg(0x12)|0x30));//make sure the Yoffset control is opened.
        for(i = 0; (0xFF != sensor_reg_ptr[i].reg_addr) || (0xFF != sensor_reg_ptr[i].reg_value); i++)
        {
                HI253_WriteReg(sensor_reg_ptr[i].reg_addr, sensor_reg_ptr[i].reg_value);
        }
        msleep(100); 
...
LOCAL uint32_t HI253_set_contrast(uint32_t level)
{
        ...
        sensor_reg_ptr = (SENSOR_REG_T*)HI253_contrast_tab[level];

        if(level >= 7) {
                SENSOR_PRINT("HI253_set_contrast:param error,level=%d .\n",level);
                return SENSOR_FAIL;
        }	
...
LOCAL uint32_t set_hi253_preview_mode(uint32_t  preview_mode)
{
        SENSOR_PRINT("set_hi253_preview_mode: preview_mode = %d", preview_mode);
...
LOCAL uint32_t HI253_set_image_effect(uint32_t effect_type)
{
        ...
        SENSOR_PRINT("HI253_set_image_effect: effect_type = %d", effect_type);
        return 0;
        }
...
LOCAL uint32_t HI253_Before_Snapshot(uint32_t param)
{      
        SENSOR_PRINT("sensor:HI253_Before_Snapshot"); 
        ...
        if(SENSOR_MODE_SNAPSHOT_ONE_FIRST == param) {
                ...
                SENSOR_PRINT("HI253_Before_Snapshot:dont set any");
        }
        SENSOR_PRINT("sensor:HI253_Before_Snapshot2");
        return 0;
}
#if 0
LOCAL uint32_t HI253_read_ev_value(uint32_t value)
...
LOCAL uint32_t HI253_set_work_mode(uint32_t mode)
{
        ...
        if(mode > 1) {
                SENSOR_PRINT("HI253_set_work_mode:param error,mode=%d .\n",mode);
                return SENSOR_FAIL;
        }	
        for(i = 0; (0xFF != sensor_reg_ptr[i].reg_addr) || (0xFF != sensor_reg_ptr[i].reg_value); i++) {
                Sensor_WriteReg(sensor_reg_ptr[i].reg_addr, sensor_reg_ptr[i].reg_value);
        }
        SENSOR_PRINT("sensor:HI253_set_work_mode: mode = %d", mode);
        return 0;
}
...
const SENSOR_REG_T HI253_WB_mode_tab[][11]=
{
        //AUTO
        {
                ...
                {0x85, 0x60}, //54 //59 //55
                {0x86, 0x23}, //0x20 --> 0x15 (by À±Ã¥ÀÓ´Ô, 20100513)
                {0xff, 0xff}
        },    
        //INCANDESCENCE:
        {
...
LOCAL uint32_t HI253_set_whitebalance_mode(uint32_t mode )
{
        ...
        if(mode > 6) {
                SENSOR_PRINT("HI253_set_whitebalance_mode:param error,mode=%d .\n",mode);
                return SENSOR_FAIL;
        }
        ...
        SENSOR_PRINT("HI253_set_whitebalance_mode: mode = %d", mode);
        return 0;
}
/******************************************************************************/
...
//		 
/******************************************************************************/
const SENSOR_REG_T HI253_video_mode_nand_tab[][52]=
{
 {
	{0x01, 0xf1}, //sleep off	
	{0x0e, 0x00}, //PLL off
	...
	{0x03, 0x00},//20091015                                
	{0x0e, 0x05},//20091015 3)PLL off                      
	{0x0e, 0x75},//20091015 4)PLL x2, on  
...
{	
        ...
        msleep(150);
        SENSOR_PRINT("SENSOR: set_hi253_video_mode: mode = %d", mode);
        return 0;
}
...
LOCAL uint32_t set_hi253_ev(uint32_t level)
{
        ...
        if(level >= 7) {
                SENSOR_PRINT("Hi253 set_hi253_ev:param error,level=%d .\n",level);
                return SENSOR_FAIL;
        }
        ...
        SENSOR_PRINT("SENSOR: set_hi253_ev: level = %d", level);
        return 0;
}
