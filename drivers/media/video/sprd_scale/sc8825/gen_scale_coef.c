		      ...
		      void *temp_buf_ptr, uint32_t temp_buf_size)
{
	...
	uint32_t coef_buf_size =
	    COEF_ARR_ROWS * COEF_ARR_COLUMNS * sizeof(int16_t);
	...
	coef_len =
	    CalUV_ScalingCoef(4, D_hor, I_hor, temp_filter_ptr, 1, &pool);
	GetFilter(temp_filter_ptr, filter_ptr, 8, coef_len, filter_len);
	...
	coef_len =
	    CalUV_ScalingCoef((int16_t) (tap), D_ver, I_ver, temp_filter_ptr, 0,
			      &pool);
	GetFilter(temp_filter_ptr, filter_ptr, 8, coef_len, filter_len);
