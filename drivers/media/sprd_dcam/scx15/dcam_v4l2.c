			...
			struct v4l2_buffer *p)
{
	...
	DCAM_TRACE("V4L2: dqbuf, index 0x%x addr 0x%x 0x%x \n", node.index,
		path->frm_addr[node.index-path->frm_id_base].yaddr,
		path->frm_addr[node.index-path->frm_id_base].uaddr);
