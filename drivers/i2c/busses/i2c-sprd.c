...
sprd_i2c_poll_ctl_status (struct sprd_i2c *pi2c, unsigned long bit)
{
  ...
  while (!(__raw_readl (pi2c->membase + I2C_CTL) & bit) && (--loop_cntr > 0));
...
sprd_i2c_poll_cmd_status (struct sprd_i2c *pi2c, unsigned long bit)
{
  ...
  while ((__raw_readl (pi2c->membase + I2C_CMD) & bit) && (--loop_cntr > 0));

  return (loop_cntr > 0);
}

static inline int
sprd_i2c_wait_int (struct sprd_i2c *pi2c)
{
  return sprd_i2c_poll_ctl_status (pi2c, I2C_CTL_INT);
}

static inline int
sprd_i2c_wait_busy (struct sprd_i2c *pi2c)
{
  return sprd_i2c_poll_cmd_status (pi2c, I2C_CMD_BUSY);
}

static inline int
sprd_i2c_wait_ack (struct sprd_i2c *pi2c)
{
  return sprd_i2c_poll_cmd_status (pi2c, I2C_CMD_ACK);
}

static inline void
sprd_i2c_clear_int (struct sprd_i2c *pi2c)
{
  unsigned int cmd = 0;
...
static inline int
sprd_wait_trx_done (struct sprd_i2c *pi2c)
{
  int rc;
...
static int
sprd_i2c_write_byte (struct sprd_i2c *pi2c, char byte, int stop,
		     int is_last_msg)
{
  int rc = 0;
  int cmd;
...
static int
sprd_i2c_read_byte (struct sprd_i2c *pi2c, char *byte, int stop)
{
  int rc = 0;
  int cmd;
		     ...
		     int is_last_msg)
{
  ...
  for (ii = 0; rc >= 0 && ii != count; ++ii)
    rc = sprd_i2c_write_byte (pi2c, buf[ii], ii == count - 1, is_last_msg);
  return rc;
}
		     ...
		     int is_last_msg)
{
  ...
  if ((pmsg->flags & I2C_M_RD))
    ...
    {
      return sprd_i2c_writebytes (pi2c, pmsg->buf, pmsg->len, is_last_msg);
    }
}
...
static u32
sprd_i2c_func (struct i2c_adapter *adap)
{
...
static void
sprd_i2c_set_clk (struct sprd_i2c *pi2c, unsigned int freq)
{
...
void
sprd_i2c_ctl_chg_clk (unsigned int id_nr, unsigned int freq)
{
  unsigned int tmp;
  ...
  tmp = __raw_readl (sprd_i2c_ctl_id[id_nr]->membase + I2C_CTL);
  __raw_writel (tmp | I2C_CTL_EN, sprd_i2c_ctl_id[id_nr]->membase + I2C_CTL);
  tmp = __raw_readl (sprd_i2c_ctl_id[id_nr]->membase + I2C_CTL);
}
...
static void
sprd_i2c_reset (struct sprd_i2c *pi2c)
{
  /*enable i2c clock */
...
static void
sprd_i2c_enable (struct sprd_i2c *pi2c)
{
  unsigned int tmp;
...
static int
sprd_i2c_remove (struct platform_device *pdev)
{
