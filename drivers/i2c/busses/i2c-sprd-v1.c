...
static struct sprd_i2c *sprd_i2c_ctl[10];//set to 10 
...
void sprd_i2c_set_clk(unsigned int id_nr, unsigned int freq)
{
  	unsigned int tmp;
	...
	//printk("I2C:sprd_i2c_set_clk: disable i2c%d, i2c ctl:%x\n", id_nr, tmp);
    	set_i2c_clk(sprd_i2c_ctl[id_nr], freq);

	tmp=__raw_readl(sprd_i2c_ctl[id_nr]->membase+I2C_CTL);
	__raw_writel(tmp|I2C_CTL_EN,sprd_i2c_ctl[id_nr]->membase+I2C_CTL);
	tmp=__raw_readl(sprd_i2c_ctl[id_nr]->membase+I2C_CTL);
	//printk("I2C:sprd_i2c_set_clk: enable i2c%d, i2c ctl:%x\n", id_nr, tmp);
}
EXPORT_SYMBOL_GPL(sprd_i2c_set_clk);
...
void sprd_i2c_ctl_chg_clk(unsigned int id_nr, unsigned int freq)
{
	...
	sprd_i2c_set_clk(sprd_i2c_ctl[id_nr], freq);
