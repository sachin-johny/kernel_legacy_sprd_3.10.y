...
static unsigned int sprd_rfspi_write(u16 Addr, u16 Data)
{
	...
	writel(reg_data,RFSPI_MCU_WCMD);
...
static unsigned int sprd_rfspi_read(u16 Addr, u32 *Read_data)
{
	...
	writel(reg_data,RFSPI_MCU_RCMD);

	sprd_rfspi_wait_read_idle();
	*Read_data = readl(RFSPI_MCU_RDATA);
	return 0;
}
...
static void sprd_mspi_disable(void)
{
	...
	/*Enable mspi function*/
	sprd_reg_read(APB_EB0_BASE_VA,&value);
	value &= ~(1<<23);
	sprd_reg_write(APB_EB0_BASE_VA,value);
}
