	...
	struct i2c_client *client)
{
	...
	dev_info(&client->dev, "akm8975 driver check i2c pass.\n");
...
{
	uint32_t ready;
	AKM_DATA(&akm->i2c->dev, "AKM8975 %s: flag =0x%X", __func__,
		rbuf[0]);
...
static irqreturn_t akm8975_irq(int irq, void *handle)
{
	...
	int err;
        ...
        AKM_DATA(&akm->i2c->dev, "%s i will read data. \n", __func__);
	...
	if ((buffer[0] & 0x01) != 0x01) {
		AKM_DATA(&akm->i2c->dev, "%s ST is not set.", __func__);
		goto work_func_end;
	}
	...
	wake_up(&akm->drdy_wq);
        ...
        AKM_DATA(&akm->i2c->dev, "%s i have read data, leaving..\n", __func__);
work_func_end:
	return IRQ_HANDLED;
}
...
static void akm8975_delayed_work(struct work_struct *work)
{
    ...
    dev_dbg(&akm->i2c->dev, "%s", __func__);
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (gpio_request(MSENSOR_DRDY_GPIO, "MSENSOR_RDY")) {
		pr_err("gpio request error for MSENSOR_RDY!\n");
		goto exit4;
	}

	s_akm->irq = gpio_to_irq(MSENSOR_DRDY_GPIO);
