...
static int rtfled_set_strobe_brightness(struct flashlight_device *flashlight_dev,
                                        int brightness_sel)
{
    rt_fled_info_t *info = flashlight_get_data(flashlight_dev);
...
static int rtfled_list_color_temperature(struct flashlight_device *flashlight_dev,
                                         int selector)
{
    /* Doesn't support color temperature */
    return -EINVAL;
...
static int __devinit rtfled_probe(struct platform_device *pdev)
{
...
static int __devexit rtfled_remove(struct platform_device *pdev)
{
...
static struct platform_driver rt_flash_led_driver = {
	...
	.probe		= rtfled_probe,
	.remove		= __devexit_p(rtfled_remove),
};
