...
void check_if_wait_trace(int id)
{
	while (is_wait_trace == id) ;
}
...
static void print_ahb(void)
{
	...
	val = sprd_greg_read(REG_TYPE_AHB_GLOBAL, AHB_CTL6);
	printk("##: AHB_CTL6 = %08x.\n", val);
	if (val & BIT_DISPC_EB) printk("BIT_DISPC_EB =1.\n");
	if (val & BIT_EMMC_EB) printk("BIT_EMMC_EB =1.\n");
	if (val & BIT_SDIO2_EB) printk("BIT_SDIO2_EB =1.\n");
}
...
static void print_gr(void)
{
	...
	val = sprd_greg_read(REG_TYPE_GLOBAL, GR_CLK_GEN6);
	printk("##: GR_CLK_GEN6 = %08x.\n", val);
	if (val & BIT_IIS2_EB) printk("GEN6_IIS2_EB =1.\n");
	if (val & BIT_SPI3_EB) printk("GEN6_SPI3_EB =1.\n");
	if (val & BIT_SPI2_EB) printk("GEN6_SPI2_EB =1.\n");

	val = sprd_greg_read(REG_TYPE_GLOBAL, GR_CLK_GEN7);
	printk("##: GR_CLK_GEN7 = %08x.\n", val);
	if (val & BIT_UART3_EB) printk("GEN7_UART3_EB =1.\n");
	if (val & BIT_I2C4_EB) printk("GEN7_I2C4_EB =1.\n");
	if (val & BIT_AUD_TOP_EB) printk("GEN7_AUD_TOP_EB =1.\n");
	if (val & BIT_AUD_IF_EB) printk("GEN7_AUD_IF_EB =1.\n");
...
static void debugfs_init(void)
{
	...
	debugfs_create_u32("print_time", 0666, dentry_debug_root,
			   &is_print_time);
	debugfs_create_u32("wait_trace", 0666, dentry_debug_root,
			   &is_wait_trace);
}
