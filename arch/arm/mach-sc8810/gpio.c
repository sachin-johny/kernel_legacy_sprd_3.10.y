...
#define GPIO_INVALID_ID (0xffffffff)
...
static void __get_gpio_base_info(u32 gpio_id, struct gpio_info *info)
{
	...
	pr_err("__get_gpio_base_info Error id = %d\n", gpio_id);

	return;
}

static int __gpio_set_input_enable(struct gpio_info *info, int enable)
{
	int value = 0;
	int __enable = ! !enable;
	u32 reg_addr = 0;
...
static int sprd_gpio_direction_input(struct gpio_chip *chip, unsigned offset)
{
	...
	if (res < 0)
		goto Err;
	res = __gpio_set_input_enable(&gpio_info, 1);
	if (res < 0)
		goto Err;
	...
	*/
	...
	GPIO_DBG
	    ("gpio info, pin is :%d, base addr :%x, bit num :%d, type :%d\r\n",
	     gpio_id, gpio_info.base_addr, gpio_info.bit_num,
	     gpio_info.gpio_type);
...
static int __gpio_get_int_status(struct gpio_info *info)
{
	...
	case GPIO_SECTION_GPO:
		WARN(1,
		     "The corresponding reg of this GPIO_ID is a GPO! No Intr!");
		return false;
	case GPIO_SECTION_INVALID:
	default:
		WARN(1, "the GPIO_ID is Invalid in this chip");
...
static void __gpio_set_int_type(struct gpio_info *info, int type)
{
	if (info->gpio_type == GPIO_SECTION_GPI) {
		...
		switch (type) {
		case IRQ_TYPE_LEVEL_HIGH:	// detect high level.
			gpio_reg_or((info->base_addr + GPIO_IS), info->die,
				    (0x1 << info->bit_num));
			gpio_reg_and(info->base_addr + GPIO_IBE, info->die,
				     ~(0x1 << info->bit_num));
			gpio_reg_or((info->base_addr + GPIO_IEV), info->die,
				    (0x1 << info->bit_num));
			break;
		...
		case IRQ_TYPE_LEVEL_LOW:	// detect low level.
			...
			gpio_reg_and(info->base_addr + GPIO_IBE, info->die,
				     ~(0x1 << info->bit_num));
			gpio_reg_and((info->base_addr + GPIO_IEV), info->die,
				     ~(0x1 << info->bit_num));
			break;

		case IRQ_TYPE_EDGE_BOTH:	// detect the rising edges and falling edges.
			gpio_reg_and((info->base_addr + GPIO_IS), info->die,
				     ~(0x1 << info->bit_num));
			gpio_reg_or(info->base_addr + GPIO_IBE, info->die,
				    (0x1 << info->bit_num));
			gpio_reg_and((info->base_addr + GPIO_IEV), info->die,
				     ~(0x1 << info->bit_num));
			break;
...
void gpio_set_hw_debounce(unsigned int gpio, u8 period)
{
	...
	if (period >= 1) {
		...
		value =
		    (gpio_reg_get(reg_addr, gpio_info.die) & 0xFF00) + period;
...
static void sprd_gpio_irq_init(void)
{
	...
	gpio_mux_handler = sprd_gpio_demux_handler;
