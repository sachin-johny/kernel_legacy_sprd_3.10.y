...
ssize_t mux_ringbuffer_read(struct mux_ringbuffer *rbuf, u8 *buf, size_t len, int usermem)
{
	size_t todo = len;
	size_t split;
...
ssize_t mux_ringbuffer_write(struct mux_ringbuffer *rbuf, const u8 *buf, size_t len)
{
	size_t todo = len;
	size_t split;
...
ssize_t mux_ringbuffer_pkt_write(struct mux_ringbuffer *rbuf, u8* buf, size_t len)
{
	int status;
...
void mux_ringbuffer_pkt_dispose(struct mux_ringbuffer *rbuf, size_t idx)
{
	...
	while(mux_ringbuffer_avail(rbuf) > MUX_RINGBUFFER_PKTHDRSIZE) {
		if (MUX_RINGBUFFER_PEEK(rbuf, 2) == PKT_DISPOSED) {
			...
			MUX_RINGBUFFER_SKIP(rbuf, pktlen + MUX_RINGBUFFER_PKTHDRSIZE);
		} else {
...
ssize_t mux_ringbuffer_pkt_next(struct mux_ringbuffer *rbuf, size_t idx, size_t* pktlen)
{
	int consumed;
	int curpktlen;
	int curpktstatus;
	...
	if (idx == -1) {
		...
		idx = (idx + curpktlen + MUX_RINGBUFFER_PKTHDRSIZE) % rbuf->size;
	}

	consumed = (idx - rbuf->pread) % rbuf->size;

	while((mux_ringbuffer_avail(rbuf) - consumed) > MUX_RINGBUFFER_PKTHDRSIZE) {
