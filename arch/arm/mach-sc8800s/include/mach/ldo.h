...
#define REG_SETCLRBIT(_reg_addr, _set_bit, _clr_bit)	\
	do	\
	{	\
		reg_val = *(volatile unsigned int*)(_reg_addr);	\
        	reg_val |= (_set_bit);	\
        	reg_val &= ~(_clr_bit);	\
        	*(volatile unsigned int*)(_reg_addr) = reg_val;	\
        }while(0);
...
#define GET_LEVELBIT(_reg_addr, _bit_mask, _bit_set, _level_var)   \
	{	\
		(_level_var) |=	\
		((*(volatile unsigned int*)(_reg_addr)&(_bit_mask))?(_bit_set):0);	\
	}
...
#define GET_LEVEL(_reg_addr, _bit0_mask, _bit1_mask, _level_var)    \
        {	\
        	(_level_var) |=	\
        	(((*(volatile unsigned int*)(_reg_addr)&(_bit0_mask))?(BIT_0):0)|    \
        	((*(volatile unsigned int*)(_reg_addr)&(_bit1_mask))?(BIT_1):0));	\
        }	
