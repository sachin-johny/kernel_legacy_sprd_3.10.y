...
#ifdef CONFIG_SPRD_DEBUG
    sprd_debug_irq_log(0, nk_do_local_timer, 1);
#endif
    evt->event_handler(evt);
#ifdef CONFIG_SPRD_DEBUG
   sprd_debug_irq_log(0, nk_do_local_timer, 2);
#endif
    return IRQ_HANDLED;
}
...
asmlinkage void __exception_irq_entry do_local_timer(struct pt_regs *regs)
{
	...
	if (local_timer_ack()) {
		__inc_irq_stat(cpu, local_timer_irqs);
#ifdef CONFIG_SPRD_DEBUG
		sprd_debug_irq_log(0, do_local_timer, 1);
#endif
		ipi_timer();
#ifdef CONFIG_SPRD_DEBUG
		sprd_debug_irq_log(0, do_local_timer, 2);
#endif
	}
#ifdef CONFIG_SPRD_DEBUG
	else
		sprd_debug_irq_log(0, do_local_timer, 3);
#endif
...
#ifdef CONFIG_SPRD_DEBUG
	sprd_debug_irq_log(ipinr, nk_do_IPI, 1);
#endif
