...
#define EMC_DLL_SET_BPS_200			0x3
#define EMC_DLL_CLR_BPS_200			0x4
...
static void ddr_cam_command_dequeue(uint32 isEnable)
{
    if(isEnable)
    {
...
static void ddr_timing_update(ddr_dfs_val_t *timing_param)
{
...
static void exit_lowpower_mode(void)
{
	...
	*(volatile uint32*)(UMCTL_PWRCTL) = 0;
...
static void ddr_clk_set(uint32 new_clk, uint32 dll_isEnable,ddr_dfs_val_t *timing_param)
{
    volatile uint32 i;
    
//    port_disable();
    //enter selfrefresh
//    umctl_enter_state_selfrefresh(1);
    ...
    *(volatile uint32*)UMCTL_DFIMISC &= ~0x1;
    ...
    //close ddr phy clock
    *(volatile uint32*)0x022b00c8 &= ~(1<<6);
//    *(volatile uint32*)0x022b00c8 &= ~(1<<2);
	...
	switch(new_clk)
	{
            case 192:
            {
                ...
                reg_bits_set(0x022d0024,0x8,2,0x1);

                for(i=0;i<0x2;i++);

                //sel tdpll source 384Mhz
                reg_bits_set(0x022d0024,0x0,2,0x2);
...
//              *(volatile uint32*)0x022b00c8 |= (1<<2);
                ...
                //open automatic selfrefresh
                *(volatile uint32*)UMCTL_DFILPCFG0 = 0x0700f100;
                *(volatile uint32*)UMCTL_PWRCTL    = 0x1;
                
                 //first update timing parameter
                ddr_timing_update(timing_param);
               break;
            }
            ...
            {
                //set emc clock divider
                reg_bits_set(0x022d0024,0x8,2,0x0);

                for(i=0;i<0x2;i++);

                //sel tdpll source 533Mhz
                reg_bits_set(0x022d0024,0x0,2,0x3);
...
//              *(volatile uint32*)UMCTL_DFILPCFG0 = 0x0700f000;
//              *(volatile uint32*)UMCTL_PWRCTL    = 0x0;
                break;
            }
            default:
            	break;
    ...
    *(volatile uint32*)PUBL_PIR |= ((1<<4)|(1<<0));
    while(((*(volatile uint32*)PUBL_PGSR) & 0x1) != 0x1);
    while(((*(volatile uint32*)PUBL_PGSR) & 0x1) != 0x1);
    *(volatile uint32*)PUBL_PIR |= (1<<4);

    *(volatile uint32*)UMCTL_DFIMISC |= (1<<0);
//    umctl_enter_state_selfrefresh(0);
//    port_enable();
...
#define MCU_XTLEN_AUTOPD_EN	BIT_12
...
#define APB_SLP_CTL	(0x40010000 + 0x44)
	REG32(AHB_SLP_CTL) |= MCU_XTLEN_AUTOPD_EN;
	REG32(AHB_SLP_CTL) |= (MCUMTX_AUTO_GATE_EN) | (MCU_AUTO_GATE_EN) | (AHB_AUTO_GATE_EN) | (ARM_AUTO_GATE_EN) | (ARM_DAHB_SLEEP_EN);
}
static void dsp_deep_sleep(void)
{
	...
	REG32(0X50130030) |= (BIT_15) | (BIT_13); //Td_slp_poweroff_auto_en, GSM_slp_poweroff_auto_en
	REG32(0X50130014) |= BIT_27; //enable RFT DSP clock
	REG32(0X50750108) |= BIT_21; //set RFT_RFLDO_AUTO_PD=1
	REG16(0X50640256) |= (BIT_15);//set RFT_RFLDO_AUTO_PD = 1
	REG32(0X50750104) |= (BIT_21);//set RFT_RFXTL_AUTO_PD = 1
	REG16(0X50640254) |= (BIT_15);
	REG32(0X50750094) |= (BIT_15);//set ecc_force_off = 1
	REG16(0X506402EA) |= (BIT_12);
	REG32(0X50130014) &= ~(BIT_27);
...
static void clk_config(void)
{
	...
	for(i = 0; i < 20; i++);
	REG32(0x2010004c) |= (3 << 0);//ARM clk set to 307.2M WPLL
}
...
void main_entry(void)
{
	...
	while(1){
		...
		ddr_clk_set(clk, dll_mode, timing);
