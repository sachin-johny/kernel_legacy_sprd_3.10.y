...
/* debugging macros */
#define CPU_FREQ_INFO(x...)         pr_info("cpufreq-sprd: " x)
#define CPU_FREQ_DEBUG(x...)        pr_debug("cpufreq-sprd: " x)
#define CPU_FREQ_ERR(x...)          pr_err("cpufreq-sprd: error: " x)
...
static void set_mcu_clk_freq(u32 mcu_freq)
{
	...
	CPU_FREQ_DEBUG("%s --- before, AHB_ARM_CLK: %08x, rate = %d, div = %d\n",
	...
	CPU_FREQ_DEBUG("%s --- after, AHB_ARM_CLK: %08x, rate = %d, div = %d\n",
		__func__, __raw_readl(REG_AHB_ARM_CLK), rate, arm_clk_div);
...
/* khz */
static unsigned int sprd_top_frequency;
...
/* ns */
#define TRANSITION_LATENCY	(100 * 1000)
...
#define CPUFREQ_SET_VOLTAGE() \
	do { \
	    ret = regulator_set_voltage(sprd_cpufreq_conf->regulator, \
			...
			sprd_cpufreq_conf->vddarm_mv[index]); \
		if (ret) \
			CPU_FREQ_ERR("Failed to set vdd to %d mv\n", \
				sprd_cpufreq_conf->vddarm_mv[index]); \
	} while (0)
#define CPUFREQ_SET_CLOCK() \
	do { \
		if (freq->new == SHARK_TDPLL_FREQUENCY) { \
			...
			if (ret) \
				CPU_FREQ_ERR("Failed to set cpu parent to tdpll\n"); \
		} else { \
			...
			if (ret) \
				CPU_FREQ_ERR("Failed to set mpll rate\n"); \
			ret = clk_set_parent(sprd_cpufreq_conf->clk, sprd_cpufreq_conf->mpllclk); \
			if (ret) \
				CPU_FREQ_ERR("Failed to set cpu parent to mpll\n"); \
		} \
	} while (0)
	...
	CPU_FREQ_INFO("%u --> %u, real=%u, index=%d\n",
...
static void sprd_real_set_cpufreq(int cpu, unsigned int new_speed, int index)
{
	...
	CPU_FREQ_DEBUG("perpare set %u khz for cpu%u\n", new_speed, cpu);
...
static int sprd_cpufreq_verify_speed(struct cpufreq_policy *policy)
{
	if (policy->cpu > CONFIG_NR_CPUS) {
		CPU_FREQ_ERR("%s no such cpu id %d\n", __func__, policy->cpu);
		return -EINVAL;
	}
		       ...
		       unsigned int relation)
{
	...
	if (cpufreq_frequency_table_target(policy, table,
		...
		CPU_FREQ_ERR("invalid target_freq: %d\n", target_freq);
		return -EINVAL;
	}

	CPU_FREQ_DEBUG("CPU_%d target %d relation %d (%d-%d) selected %d\n",
	...
	CPU_FREQ_DEBUG("%s cpu:%d new_speed:%u on cpu%d\n",
...
static int sprd_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	if (ret != 0)
		CPU_FREQ_ERR("%s Failed to config freq table: %d\n", __func__, ret);

	CPU_FREQ_DEBUG("%s policy->cpu=%d, policy->cur=%u, ret=%d\n",
