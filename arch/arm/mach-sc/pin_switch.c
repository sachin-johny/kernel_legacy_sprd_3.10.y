...
{
	return single_open(file, pin_switch_proc_show, PDE(inode)->data);
}
				     ...
				     size_t count, loff_t * pos)
{
	...
	struct sci_pin_switch *p =
	    (struct sci_pin_switch *)(PDE(file->f_path.dentry->d_inode)->data);
...
{
	return single_open(file, pin_switch_dir_proc_show, PDE(inode)->data);
}
