...
static struct sys_timer sc8830_timer = {
	.init = sci_timer_init,
};
...
const char * sc8830_regulator_map[] = {
...
int __init sc8830_regulator_init(void)
{
	static struct platform_device sc8830_regulator_device = {
		.name 	= "sprd-regulator",
		.id	= -1,
		.dev = {.platform_data = sc8830_regulator_map},
	};
	return platform_device_register(&sc8830_regulator_device);
}

static int __init sc8830_clock_init_early(void)
{
...
//		BIT_PWM0_EB			|
	...
	printk("sc8830 clock module init ok\n");
	return 0;
}
