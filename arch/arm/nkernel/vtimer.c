...
vtimer_legacy_setup (void)
{
    ...
    DTRACE("clockevent shift=%d mult=%lu\n", vtimer_clockevent.shift,
...
vtimer_setup (void)
{
    ...
    if (!vtimer.c_xirq) {
	ETRACE("VTIMER: Unable to allocate dynamic XIRQ\n");
	return 0;
    }
    ...
    if (!vtimer.id) {
	ETRACE("VTIMER: Unable to allocate NK timer\n");
	return 0;
    }
    ...
    DTRACE("VTIMER: NK CBPS interface used\n");
