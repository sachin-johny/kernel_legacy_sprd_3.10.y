...
{
    NkResource  resrc;
    NkDevVlink* vlink = (NkDevVlink*)PTOV(link);
...
nk_cpu_id_get (void)
{
    DKI_PANIC(("nk_cpu_id_get called\n"));
    return 0;
}
...
{
    DKI_PANIC(("NKDDI: nk_xvex_post called\n"));
}

    /*
     * Get scheduling parameters of a given operating system
     */
    static void
nk_scheduler (NkOsId osId, NkSchedParams* oldp, NkSchedParams* newp)
{
    DKI_PANIC(("NKDDI: nk_scheduler called\n"));
}
...
{
    DKI_PANIC(("nk_machine_addr called\n"));
    return 0;
}
...
{
    DKI_PANIC(("nk_log_buffer called\n"));
    return 0;
}
...
{
    DKI_PANIC(("nk_log_guests called\n"));
}
...
{
    DKI_PANIC(("nk_log_events called\n"));
}
...
{
    DKI_PANIC(("nk_cmdline_get called\n"));
    return 0;
}
...
{
    DKI_PANIC(("nk_mem_map_sync called\n"));
    return 0;
}
...
{
    DKI_PANIC(("nk_mem_unmap_sync called\n"));
}
...
{
    DKI_PANIC(("nk_cache_sync called\n"));
}
...
{
    XIrqWrapper*  wrapper = (XIrqWrapper*)id;
...
nk_xirq_unmask (NkXIrq xirq)
{
    ...
    inline void
__nk_xirq_mask (NkXIrq xirq)
{
    _xirq_emask_clear(&nkdev.xenabled, xirq);
}

    inline void
__nk_xirq_unmask (NkXIrq xirq)
{
    _xirq_emask_set(&nkdev.xenabled, xirq);
}

    inline void
__nk_xirq_ack (NkXIrq xirq)
{
}

    inline unsigned int
__nk_xirq_startup(NkXIrq xirq)
{
    _xirq_emask_set(&nkdev.xenabled, xirq);
    return 0;
}
    ...
    inline void
__nk_xirq_shutdown(NkXIrq xirq)
{
    _xirq_emask_clear(&nkdev.xenabled, xirq);
}
...
{
    DKI_PANIC(("nk_local_xirq_post called\n"));
}
...
#ifdef CONFIG_SMP
    static int
__nk_set_cpu (unsigned int irq, const struct cpumask* mask)
{
    nk_xirq_affinity(irq, cpumask_bits(mask)[0]);
    return 0;
}
#endif
...
struct irq_chip nk_xirq_chip = {
    ...
    .ack      = __nk_xirq_ack,
    .mask     = __nk_xirq_mask,
    .unmask   = __nk_xirq_unmask,
    .startup  = __nk_xirq_startup,
    .shutdown = __nk_xirq_shutdown,
#ifdef CONFIG_SMP
    .set_affinity = __nk_set_cpu,
#endif
};
...
nk_ddi_init (void)
{
	...
	set_irq_chip(xirq, &nk_xirq_chip);
	set_irq_handler(xirq, handle_edge_irq);
